(() => {
  ...
  window.WfGgHeroRosterV15={version:'15.0.0'};
  ...
  function byDisplayName(name){
    const n=norm(name);
    if(n==='skyler')return byId('schuyler');
    return catalog.find(h=>norm(h.name)===n)||null;
  }
  ...
  function decorateCard(card){
    ...
    const flags=card.querySelector('.wfgg-v10-flags');
    if(flags){
      const promo=flags.querySelector('.wfgg-v10-promo,.wfgg-v10-awaken')?.outerHTML||'';
      flags.innerHTML=`${icon(cat.troopType)}${icon(cat.role)}${promo}`;
      flags.dataset.wfggV15Icons='1';
    }
  }
  function decorateSheet(){
    ...
    const tags=sheet.querySelector('.hero-sheet-tags');if(!tags)return;
    const rarity=sheet.querySelector('.rarity-tag')?.textContent?.trim()||cat.rarity||'UR';
    const awakening=sheet.querySelector('.awakening-tag')?.outerHTML||'';
    tags.innerHTML=`<span class="hero-sheet-tag wfgg-v15-sheet-icon">${icon(cat.troopType)}</span><span class="hero-sheet-tag wfgg-v15-sheet-icon">${icon(cat.role)}</span><span class="hero-sheet-tag rarity-tag rarity-${rarity}">${rarity}</span>${awakening}`;
    tags.dataset.wfggV15Icons='1';
  }
  ...
  function rebuild(){
    ...
    visibleCards().forEach(c=>delete c.dataset.wfggV15Seen);
    document.documentElement.dataset.wfggV15CycleSize=String(queue.length);
  }
  function clearActive(){active.forEach(c=>c?.classList.remove('wfgg-v15-live'));active=[]}
  function rest(){
    clearActive();stripLegacy();
    document.documentElement.dataset.wfggV15Phase='rest';
  ...
  function startBatch(){
    ...
    document.documentElement.dataset.wfggV15Phase='active';
    document.documentElement.dataset.wfggV15Batch=active.map(c=>c.dataset.heroId).join(',');
    active.forEach((card,i)=>{
      decorateCard(card);
      card.style.setProperty('--wfgg-v15-delay',`${i*32}ms`);
      card.classList.add('wfgg-v15-live');
      card.dataset.wfggV15Seen='1';
    });
  ...
  async function init(){
    ...
    const mo=new MutationObserver(muts=>{
      let rescan=false;
      for(const m of muts){
        if(m.type==='attributes'&&m.target?.classList?.contains('game-hero-card')&&LEGACY.some(k=>m.target.classList.contains(k)))LEGACY.forEach(k=>m.target.classList.remove(k));
        if(m.type==='childList')rescan=true;
      }
      if(rescan)queueScan();
    });
