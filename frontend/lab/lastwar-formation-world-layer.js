...
const RUNTIME_VIDEO='local_assets/lastwar-formation-runtime-video-v1/formation-transition-v1.mp4';
...
const image=src=>new Promise((ok,no)=>{const i=new Image();i.onload=()=>ok(i);i.onerror=()=>no(Error(src));i.src=src+(src.includes('?')?'&':'?')+'v=005'});
...
const qs=new URLSearchParams(location.search),debug=qs.get('debug')==='1',forceVideo=qs.get('source')==='video';
function finish(source,detail=''){
 host.dataset.source=source;host.dataset.blurMaterial='Lapu_BlurUI002';host.dataset.blurRate=String(native.blurRate);host.dataset.blurScale=String(native.blurScale);host.dataset.radius=String(native.radius);host.appendChild(note);
...
function seekVideoFrame(src,t){return new Promise((ok,no)=>{const v=document.createElement('video');v.className='lw-world-video';v.src=src;v.preload='auto';v.playsInline=true;v.muted=true;v.controls=false;let done=false;const fail=e=>{if(done)return;done=true;no(e instanceof Error?e:Error('video runtime indisponible'))};const ready=()=>{if(!isFinite(v.duration))return;const tt=Math.max(0,Math.min(v.duration-.03,t));const onseek=()=>{if(done)return;done=true;v.pause();ok(v)};v.addEventListener('seeked',onseek,{once:true});try{v.currentTime=tt}catch(e){fail(e)}};v.addEventListener('loadedmetadata',ready,{once:true});v.addEventListener('error',fail,{once:true})})}
(async()=>{try{
 const saved=Number(localStorage.getItem('wfggFormationLayer0VideoTime'));
 if((forceVideo||Number.isFinite(saved))&&Number.isFinite(saved)){
   try{const v=await seekVideoFrame(RUNTIME_VIDEO,saved);host.replaceChildren(v);finish('native-runtime-video-frame',`t=${saved.toFixed(6)}s auto-selected`);return}catch(e){console.warn('runtime video frame unavailable',e)}
 }
 const runtime=await optionalImage(RUNTIME_WORLD);
 if(runtime){const img=document.createElement('img');img.className='lw-world-runtime';img.src=runtime.src;img.alt='';host.replaceChildren(img);finish('native-runtime-world');return}
 ...
 const layers=layerNames.map(n=>byName(manifest,n)).filter(Boolean);if(!controlMeta||!macroMeta||layers.length<2)throw Error('pack World/Terrain incomplet');while(layers.length<4)layers.push(layers[layers.length-1]);
 const ims=await Promise.all([controlMeta,macroMeta,grassMeta,...layers.slice(0,4)].map(x=>image(PACK+x.file)));const [ci,mi,gi,...li]=ims;
 ...
 const fs=`precision mediump float;varying vec2 v;uniform sampler2D c,m,g,t0,t1,t2,t3;float lum(vec3 q){return dot(q,vec3(.299,.587,.114));}void main(){vec4 w=texture2D(c,v);float z=w.r+w.g+w.b+w.a;if(z<.001)w=vec4(1.,0.,0.,0.);else w/=z;vec2 uv=v*6.0;vec3 a=texture2D(t0,uv).rgb;vec3 b=texture2D(t1,uv*1.09).rgb;vec3 d=texture2D(t2,uv*.91).rgb;vec3 e=texture2D(t3,uv*1.18).rgb;vec3 terrain=a*w.r+b*w.g+d*w.b+e*w.a;vec3 macro=texture2D(m,v*.92+vec2(.035,.04)).rgb;vec3 grass=texture2D(g,v*1.13+vec2(.08,-.03)).rgb;float shade=clamp(.88+lum(macro)*.18+(lum(grass)-.5)*.08,.82,1.08);vec3 col=mix(terrain,macro,.28);col=mix(col,grass,.08);col*=shade;gl_FragColor=vec4(col,1.0);}`;
 const sh=(type,src)=>{const q=gl.createShader(type);gl.shaderSource(q,src);gl.compileShader(q);if(!gl.getShaderParameter(q,gl.COMPILE_STATUS))throw Error(gl.getShaderInfoLog(q));return q};const pr=gl.createProgram();gl.attachShader(pr,sh(gl.VERTEX_SHADER,vs));gl.attachShader(pr,sh(gl.FRAGMENT_SHADER,fs));gl.linkProgram(pr);if(!gl.getProgramParameter(pr,gl.LINK_STATUS))throw Error(gl.getProgramInfoLog(pr));gl.useProgram(pr);
