(() => {
  ...
  function hydrateFormationMember(member) {
    ...
    // Remove the temporary troop silhouette. Pixel-master mode only accepts an
    // authentic rendered vehicle/frame extracted from the Unity model pipeline.
  ...
  function hydrateRosterCard(card) {
    ...
    // The old top-left icon was the troop family, which is not what the native
    // Last War card shows. Remove it until the authoritative role map is bound.
    card.querySelector(':scope > .lw-unit-type')?.remove();

    // The unit-family icon next to Lv.150 is valid, but switch it to the exact
    // recovered small sprite rather than a crop/synthetic icon.
