...
async function mount(host,manifest){
  ...
  for(let start=0;start<objects.length;start+=FETCH_BATCH){
    const batch=objects.slice(start,start+FETCH_BATCH);
    hud.textContent=`Chargement 3D réel… ${Math.min(start+batch.length,objects.length)}/${objects.length}`;
    const results=await Promise.allSettled(batch.map(fetchModelObject));
    results.forEach((res,i)=>{if(res.status==='fulfilled')models.push(res.value);else failures.push((batch[i]?.path||'OBJ')+': '+res.reason)});
  ...
  function draw(){
    ...
    const rv=all.map(v=>rot(v,yaw,pitch)),f=Math.min(w,h)*.56*zoom,cam=4.2;
    ...
    ctx.fillStyle='#d7deec';ctx.font='11px system-ui';ctx.fillText(`${models.length} OBJ · ${totalRaw.toLocaleString('fr-FR')} triangles${sampled?' · aperçu décimé':''}`,10,h-12);
  }
...
function centerActiveCard(behavior='smooth'){
  ...
  strip.querySelectorAll('.card').forEach(c=>c.style.scrollSnapAlign='center');
  const sr=strip.getBoundingClientRect(),ar=active.getBoundingClientRect();
  const delta=(ar.left+ar.width/2)-(sr.left+sr.width/2);
