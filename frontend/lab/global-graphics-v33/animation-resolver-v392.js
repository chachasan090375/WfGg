(()=>{
'use strict';
/* V39.4: 2D icon -> related 3D prefab resolver + exact LWGA-ID lookup.
   Exact IDs now use server-side stable_id equality and bypass the client preview filter, so the
   requested asset cannot be replaced by another correlated/token-sharing result.
*/
...
function installExactIdSearch(){
  ...
  params=function(){
    ...
    p.delete('keywords');
    p.delete('hero_id');
    p.delete('hero_relation');
    p.set('stable_id',raw.toUpperCase());
    p.set('sort','default');
    console.debug('V39_4_EXACT_ID_SEARCH',raw.toUpperCase(),'via=stable-id-equality');
    return p;
  };
...
boot();window.WFGGAnimationResolverV392={version:'39.4',state:()=>resolution,refresh:()=>{activeSid='';tick();},open:openModal};
})();