(()=>{
'use strict';
/* V39.6: 2D icon -> related 3D prefab resolver + direct exact target opening.
   Exact IDs use server-side stable_id equality. Opening a linked prefab now bypasses the generic
   search/preview pipeline entirely, then injects the exact returned asset into the viewer. This
   avoids a second-stage empty result caused by client preview/filter wrappers.
*/
...
function installExactIdSearch(){
  ...
  params=function(){
    ...
    console.debug('V39_6_EXACT_ID_SEARCH',raw.toUpperCase(),'via=stable-id-equality');
    return p;
  };
  ...
  console.info('V39_6_EXACT_ID_SEARCH installed stable-id-equality=ON preview-bypass=ON direct-open=ON');
  return true;
}
...
async function openCandidateDirect(c){
  ...
  if(!EXACT_ID.test(sid)){console.warn('V39_6_DIRECT_OPEN_INVALID_ID',sid);return;}
  closeModal();
  ...
  try{
    ...
    console.info('V39_6_DIRECT_PREFAB_OPEN',sid,'availability='+String(exact.render_availability||''),'dimension='+String(exact.dimension_class||''),'role='+String(exact.model_role||''));
  }catch(e){
    console.warn('V39_6_DIRECT_PREFAB_OPEN_FAIL',sid,e);
...
function openModal(){
  ...
  modal.innerHTML=`<div class="box"><h3>Prefab 3D lié à l’icône</h3><div class="muted">L’asset affiché est une image UI 2D. V39.6 a recherché les assets 3D partageant les identifiants de catalogue, puis a vérifié les candidats avec le scanner Unity.</div><div class="cand"><b>${esc(c.stable_id||'—')}</b><br><code>${esc(c.asset_path||c.logical_name||c.alias_name||'')}</code><div><span class="v392pill">score liaison ${esc(c.resolver_score??'—')}</span><span class="v392pill">${esc(d?.classification?.status||r.status||'à confirmer')}</span></div><div class="muted" style="margin-top:7px">${esc((c.resolver_reasons||[]).join(' · '))}</div></div><div class="row"><button class="primary" id="v392open">Ouvrir directement le prefab 3D</button><button id="v392close">Fermer</button></div></div>`;
...
boot();window.WFGGAnimationResolverV392={version:'39.6',state:()=>resolution,refresh:()=>{activeSid='';tick();},open:openModal,openDirect:openCandidateDirect};
})();