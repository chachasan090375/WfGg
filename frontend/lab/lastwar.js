(() => {
  ...
  async function load() {
    ...
    if (!token) {
      setStatus('Aucune session WfGg trouvée sur cette Preview. Revenez au portail de cette même Preview et connectez-vous.', 'error');
      return;
    }
    ...
    try {
      ...
      setStatus('Socle WfGg reconnu sur la même Preview. Une future identité Last War vérifiée pourra être reliée à ce user_id sans changer les modules.', 'success');
    } catch (error) {
