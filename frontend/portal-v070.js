...
async function api(path,options={}){const h=new Headers(options.headers||{}),hadSession=!!token();if(token())h.set('Authorization',`Bearer ${token()}`);if(options.body&&!(options.body instanceof FormData)&&!h.has('Content-Type'))h.set('Content-Type','application/json');const r=await fetch(`${cfg.API_BASE}${path}`,{...options,headers:h,cache:'no-store'});let d=null;try{d=await r.json()}catch{}if(r.status===401){clearSession();showAuth();if(hadSession&&$('authError')){$('authError').textContent=t('auth.bad');$('authError').classList.remove('hidden')}throw new Error('UNAUTHORIZED')}if(!r.ok)throw new Error(d?.error||`HTTP_${r.status}`);return d}
...
function clearSession(){localStorage.removeItem(TOKEN_KEY);Object.assign(state,{user:null,membership:null,alliance:null,system:null,permissions:null,portalSettings:{},members:[],sessions:[],profileSubpage:'main',trainRotationSummary:null,trainRotationLoading:false});}
...
function renderProfileTrainRotationCard(){
...
function openSettings(tab='profile'){$('profileMenu').classList.add('hidden');state.settingsTab=isAdmin()||tab==='profile'?tab:'profile';if(state.settingsTab==='profile')state.profileSubpage='main';$('settingsOverlay').classList.remove('hidden');renderSettings()}
