...
SRC = Path('.radar-release-src/v614-source/connector-go')


def copy_sources() -> None:
    pairs = [
        (SRC / 'internal/protocol/federated_target_v614.go', CONNECTOR / 'internal/protocol/federated_target_v614.go'),
        (SRC / 'internal/protocol/federated_target_v614_test.go', CONNECTOR / 'internal/protocol/federated_target_v614_test.go'),
        (SRC / 'cmd/radar-connector/federated_target_v614.go', CONNECTOR / 'cmd/radar-connector/federated_target_v614.go'),
        (SRC / 'cmd/radar-connector/federated_target_v614_test.go', CONNECTOR / 'cmd/radar-connector/federated_target_v614_test.go'),
    ]
...
def patch_collector() -> None:
    ...
    marker = 'WFGG_RADAR_TARGETED_FEDERATED_SERVER_V614'
    if marker in text:
        print('RADAR_V614_COLLECTOR=ALREADY_PRESENT')
        return

    old = 'players, regionDiag, err := scanCollectorRegionV67(ctx, s.game, regionScanner, token, region)'
    new = '// WFGG_RADAR_TARGETED_FEDERATED_SERVER_V614\n\t\tplayers, regionDiag, err := scanCollectorRegionTargetV614(ctx, s.game, regionScanner, token, region, query)'
    if text.count(old) != 1:
        raise SystemExit(f'V614_COLLECTOR_SCAN_ANCHOR_COUNT={text.count(old)}')
    ...
    start_new = '''radarCollectorJobs.update(jobID, func(j *collectorJob) {
		j.Status = "RUNNING"
		j.Phase = "STARTING"
		if target, ok := federatedServerTargetV614(query); ok {
			j.ServerTarget = target
		}
	})'''
    if text.count(start_old) != 1:
        raise SystemExit(f'V614_COLLECTOR_START_ANCHOR_COUNT={text.count(start_old)}')
    text = text.replace(start_old, start_new, 1)
    COLLECTOR.write_text(text, encoding='utf-8')
    print('RADAR_V614_COLLECTOR=PATCHED')


def patch_native_map_target() -> None:
    text = V4.read_text(encoding='utf-8')
    marker = 'WFGG_RADAR_TARGET_SERVER_ID_V614'
    if marker in text:
        print('RADAR_V614_NATIVE=ALREADY_PRESENT')
        return
...
\t}
'''
    new = '''\t// WFGG_RADAR_TARGET_SERVER_ID_V614: a federated Collector job may read a
...
'''
    if text.count(old) != 1:
        raise SystemExit(f'V614_NATIVE_SERVER_ANCHOR_COUNT={text.count(old)}')
    text = text.replace(old, new, 1)
    ...
    if text.count(old_diag) != 1:
        raise SystemExit(f'V614_NATIVE_DIAG_ANCHOR_COUNT={text.count(old_diag)}')
    ...
    if text.count(old_collect) != 1:
        raise SystemExit(f'V614_NATIVE_COLLECT_ANCHOR_COUNT={text.count(old_collect)}')
    text = text.replace(old_collect, new_collect, 1)

    V4.write_text(text, encoding='utf-8')
    print('RADAR_V614_NATIVE=PATCHED')


copy_sources()
patch_collector()
patch_native_map_target()
print('RADAR_TARGETED_FEDERATED_SERVER_V614=READY')
