...
new_start = "function startScan(q){screen.classList.remove('found');screen.classList.add('scanning');target.classList.remove('hit');result.classList.remove('show');go.disabled=true;steps(1);setStatus(/^@federated:/i.test(q)?'DÉMARRAGE DU CYCLE':'RECHERCHE INDEXÉE',q);tone(420,.04,.018);clearInterval(phaseTimer)}"
...
new_tail = "const fast=data?.fastLookup;const job=data?.job;const src=job?'COLLECTOR V4':fast?.status==='d1-hit'?'CACHE LOCAL':fast?.status==='exact-hit'?'INDEX IDENTITÉ':fast?.status==='fuzzy-hit'?'INDEX COLLECTOR':'RADAR';$('rsource').textContent=src;$('rraw').textContent=job?`Cycle ${job.cycleId||'—'} · ${job.joined?'rejoint':'complet'} · ${fmt(job.playersSeen||0)} observations · ${fmt(job.enriched||0)} profils`:fast?.elapsedMs!=null?`${fast.route||fast.status} · ${fast.elapsedMs} ms`:(fast?.route||'Résultat local');locate(p);result.classList.add('show');steps(4);setStatus('CIBLE LOCALISÉE',name,'ok');tone(980,.07,.03);setTimeout(()=>tone(1320,.09,.025),90)"
if old_tail not in s:
    raise SystemExit('RADAR_V611_SHOWPLAYER_ANCHOR_MISSING')
s = s.replace(old_tail, new_tail, 1)

start_marker = "$('searchForm').onsubmit=async e=>{"
start = s.find(start_marker)
if start < 0:
    raise SystemExit('RADAR_V611_SUBMIT_START_MISSING')
end_marker = "\nhealth();session();"
end = s.find(end_marker, start)
if end < 0:
    raise SystemExit('RADAR_V611_SUBMIT_END_MISSING')

new_submit = r'''/* WFGG_RADAR_FAST_LOOKUP_UI_V611 */
$('searchForm').onsubmit=async e=>{e.preventDefault();const q=$('q').value.trim();if(!q)return;const federated=/^@federated:(?:APS)?\d+/i.test(q);startScan(q);if(!federated)setStatus('RECHERCHE INDEXÉE','Pseudo → UID → fiche locale');try{const d=federated?await runCollectorSearch(q):await api('/api/radar/search?q='+encodeURIComponent(q)+'&limit=50');const p=playerFrom(d);stopScan();if(p)showPlayer(p,d,q);else if(federated){screen.classList.remove('found');result.classList.remove('show');steps(4);/* le panneau fédéré est le résultat */}else{screen.classList.remove('found');steps(4);const fast=d?.fastLookup||{};const ambiguous=fast.status==='ambiguous';setStatus(ambiguous?'PSEUDO AMBIGU':'AUCUNE CIBLE',ambiguous?`${fast.candidateCount||0} correspondance(s) · précise le serveur ou l’UID`:`« ${q} » absent de l’index local`,'error');$('rname').textContent=q;$('rserver').textContent=$('ralliance').textContent=$('rhq').textContent=$('rpower').textContent=$('rpos').textContent=$('ruid').textContent='—';$('rsource').textContent=ambiguous?'INDEX IDENTITÉ':'INDEX LOCAL';$('rraw').textContent=fast.elapsedMs!=null?`${fast.route||fast.status} · ${fast.elapsedMs} ms`:(fast.route||'Aucun résultat');result.classList.add('show');tone(180,.14,.02)}}catch(err){stopScan();screen.classList.remove('found');if(err.status===401){login.classList.add('show');setStatus('SESSION REQUISE','Connecte le Radar puis relance la recherche','error')}else if(isLastWarAuthError(err)){showReauthRequired(err.message)}else setStatus('ERREUR RADAR',err.message,'error')}};'''
