...
"""
    ...
    panel_old = "              panel.innerHTML='<h2 style=\\\"margin-top:0\\\">Train momentanément indisponible</h2><p>Ta session Portail est valide, mais les données Train n’ont pas pu être synchronisées.</p><button id=\\\"wfggTrainBootRetryV6\\\" style=\\\"padding:11px 16px;border:0;border-radius:12px;font-weight:700\\\">Réessayer</button>';\n              document.body.appendChild(panel);\n              document.getElementById('wfggTrainBootRetryV6')?.addEventListener('click',()=>location.reload());\n            }\n"
    panel_new = "              panel.innerHTML='<h2 style=\\\"margin-top:0\\\">Train momentanément indisponible</h2><p>Ta session Portail est valide, mais les données Train n’ont pas pu être synchronisées.</p><button id=\\\"wfggTrainBootRetryV6\\\" style=\\\"padding:11px 16px;border:0;border-radius:12px;font-weight:700\\\">Réessayer</button><pre id=\\\"wfggTrainInlineSentinelV15\\\" style=\\\"margin:18px 0 0;padding:14px;border-radius:12px;background:#0d0c14;border:1px solid rgba(220,196,255,.28);color:#eae5f2;text-align:left;white-space:pre-wrap;word-break:break-word;font:600 12px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace\\\">Sentinel · diagnostic en cours…</pre>';\n              document.body.appendChild(panel);\n              document.getElementById('wfggTrainBootRetryV6')?.addEventListener('click',()=>location.reload());\n            }\n            renderInlineSentinelBootV15(panel);\n"
    assert panel_old in app, 'boot panel block missing'
    app = app.replace(panel_old, panel_new, 1)
