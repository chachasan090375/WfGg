...
BASE="https://raw.githubusercontent.com/chachasan090375/WfGg/${REV}/radar-vps/pilot-v6191"
ROOT="/opt/wfgg-radar"
BIN="${ROOT}/bin"
DATA="${ROOT}/data"
SERVICE="wfgg-radar-connector"
TMP="$(mktemp -d /tmp/wfgg-radar-v6191.XXXXXX)"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP="${DATA}/v6191-pilot-backups/${STAMP}"
PROMOTED=0

log(){ printf '%s\n' "$*"; }
fail(){ log "RADAR_V6191_PILOT_INSTALL=FAIL reason=$*"; exit 1; }
cleanup(){ rm -rf "$TMP"; }
rollback(){
  rc=$?
  if [[ "$rc" -ne 0 && "$PROMOTED" -eq 1 ]]; then
    log "RADAR_V6191_PILOT_ROLLBACK=START"
    ...
    sleep 2
    systemctl is-active --quiet "$SERVICE" && log "RADAR_V6191_PILOT_ROLLBACK=PASS" || log "RADAR_V6191_PILOT_ROLLBACK=FAIL"
  fi
  cleanup
  exit "$rc"
}
...
log "=== WFGG RADAR V6.19.1 PILOT INSTALL ==="
log "RADAR_V6191_REV=$REV"
...
grep -aFq '/v1/collector/profile/refresh' "$TMP/radar-connector" || fail PROFILE_REFRESH_ROUTE_MISSING
grep -aFq 'PROFILE_TARGET_REFRESH_FAILED' "$TMP/radar-connector" || fail PROFILE_ONLY_MARKER_MISSING
...
install -m 0755 "$TMP/radar-connector" "$BIN/.radar-connector-v6191-new"
install -m 0755 "$TMP/radar-native-template" "$BIN/.radar-native-template-v6191-new"
mv -f "$BIN/.radar-connector-v6191-new" "$BIN/radar-connector"
mv -f "$BIN/.radar-native-template-v6191-new" "$BIN/radar-native-template"
PROMOTED=1
...
log "RADAR_V6191_PILOT_BACKUP=$BACKUP"
log "RADAR_V6191_PRODUCTION_MUTATION=NO"
log "RADAR_V6191_PILOT_INSTALL=PASS"
