...
func TestPlayerMailExactContractV625(t *testing.T) {
	...
	if r.Type != 21 {
		t.Fatalf("type=%d", r.Type)
	}
	if fields[3].Value != "" {
		t.Fatalf("allianceId must be empty for 1:1: %#v", fields[3])
	}
}
...
func TestOfficialUILimitsV625(t *testing.T) {
	...
	r.Title += "x"
	if !errors.Is(r.Validate(), ErrTitleTooLong) {
		t.Fatalf("expected title limit")
	}
	r = validRequest()
	r.Contents = strings.Repeat("y", 2001)
	if !errors.Is(r.Validate(), ErrContentTooLong) {
		t.Fatalf("expected content limit")
	}
}
...
func TestOfficialUIRequiresTitleAndContentV625(t *testing.T) {
	r := validRequest()
	r.Title = ""
	if !errors.Is(r.Validate(), ErrTitleRequired) {
		t.Fatal("empty title must block")
	}
	r = validRequest()
	r.Contents = ""
	if !errors.Is(r.Validate(), ErrContentRequired) {
		t.Fatal("empty content must block")
	}
}
...
func TestNoAllianceOrPresidentTypeV625(t *testing.T) {
	r := validRequest()
	r.Type = 20
	if !errors.Is(r.Validate(), ErrMailTypeInvalid) {
		t.Fatal("alliance mail must be blocked")
	}
	r.Type = 45
	if !errors.Is(r.Validate(), ErrMailTypeInvalid) {
		t.Fatal("president mail must be blocked")
	}
}
