...
REV="${WFGG_RADAR_V6193_REV:-radar-v6193-seed-scout}"
BASE="https://raw.githubusercontent.com/chachasan090375/WfGg/${REV}/radar-vps/pilot-v6193"
ROOT="/opt/wfgg-radar"
BIN="${ROOT}/bin"
DATA="${ROOT}/data"
SERVICE="wfgg-radar-connector"
TMP="$(mktemp -d /tmp/wfgg-radar-v6193.XXXXXX)"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP="${DATA}/v6193-pilot-backups/${STAMP}"
PROMOTED=0

log(){ printf '%s\n' "$*"; }
fail(){ log "RADAR_V6193_PILOT_INSTALL=FAIL reason=$*"; exit 1; }
cleanup(){ rm -rf "$TMP"; }
rollback(){
  rc=$?
  if [[ "$rc" -ne 0 && "$PROMOTED" -eq 1 ]]; then
    log "RADAR_V6193_PILOT_ROLLBACK=START"
    ...
    sleep 2
    systemctl is-active --quiet "$SERVICE" && log "RADAR_V6193_PILOT_ROLLBACK=PASS" || log "RADAR_V6193_PILOT_ROLLBACK=FAIL"
  fi
  cleanup
  exit "$rc"
}
...
log "=== WFGG RADAR V6.19.3 SEED SCOUT PILOT INSTALL ==="
log "RADAR_V6193_REV=$REV"
...
install -m 0755 "$TMP/radar-connector" "$BIN/.radar-connector-v6193-new"
install -m 0755 "$TMP/radar-native-template" "$BIN/.radar-native-template-v6193-new"
mv -f "$BIN/.radar-connector-v6193-new" "$BIN/radar-connector"
mv -f "$BIN/.radar-native-template-v6193-new" "$BIN/radar-native-template"
PROMOTED=1
...
log "RADAR_V6193_PILOT_BACKUP=$BACKUP"
log "RADAR_V6193_PRODUCTION_MUTATION=NO"
log "RADAR_V6193_PILOT_INSTALL=PASS"
