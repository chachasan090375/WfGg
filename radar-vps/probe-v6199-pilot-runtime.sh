...
fail(){ printf 'RADAR_V6199_RUNTIME_PROBE=FAIL reason=%s\n' "$1"; exit 1; }
...
echo "RADAR_V6199_CONNECTOR_SHA=$ACTUAL_CONNECTOR"
echo "RADAR_V6199_NATIVE_SHA=$ACTUAL_NATIVE"
[[ "$ACTUAL_CONNECTOR" == "$EXPECTED_CONNECTOR" ]] || fail CONNECTOR_SHA_MISMATCH
[[ "$ACTUAL_NATIVE" == "$EXPECTED_NATIVE" ]] || fail NATIVE_SHA_MISMATCH

grep -aFq 'v6.19.9' "$BIN" || fail VERSION_MARKER_MISSING
grep -aFq 'AUTOPILOT_TARGET_HISTORY_TIMEOUT_V6199' "$BIN" || fail TARGETED_HISTORY_MARKER_MISSING
...
echo "RADAR_V6199_TARGETED_HISTORY_MARKERS=PASS"
...
print("RADAR_V6199_START_ROUTE_GUARD=PASS")
...
print("RADAR_V6199_STATUS_ROUTE_GUARD=PASS")

status,payload=request("POST","/v1/collector/autopilot/stop",{"id":"does-not-exist"})
assert status==404,(status,payload)
assert payload.get("error")=="AUTOPILOT_JOB_NOT_FOUND",payload
print("RADAR_V6199_STOP_ROUTE_GUARD=PASS")
PY

echo "RADAR_V6199_GAME_SCAN_EXECUTED=NO"
echo "RADAR_V6199_COLLECTOR_MUTATION=NO"
echo "RADAR_V6199_RUNTIME_PROBE=PASS"
