...
# WfGg Radar — READONLY player scan v4 installer.
# V4 synthesizes world.get.block directly from the documented SFS wire format;
# no captured map request and no VPN recapture are required. It retains
# get.user.info.multi enrichment from the existing PCAP when available.
...
say "=== WfGg Radar · Player Scan READONLY v4 ==="
...
say "2/6 Téléchargement de la release v4"
...
if ! grep -aFq 'native-template-readonly-v4' "$TMP/radar-native-template"; then
  die "la release publiée n'est pas encore la v4"
fi
say "RELEASE_SHA256=OK"

say "3/6 Transfert chiffré vers ChaChaVPS"
TAG="$$"
RCAP="/tmp/wfgg-radar-v4-$TAG.pcap"
RCON="/tmp/wfgg-radar-connector-v4-$TAG"
RNAT="/tmp/wfgg-radar-native-v4-$TAG"
...
say "5/6 Activation du moteur READONLY v4"
PYHELPER="$TMP/install-v4-context.py"
cat > "$PYHELPER" <<'PY'
...
print('PLAYER_SCAN_V4_CONTEXT=INSTALLED')
print('CAPTURED_MAP_TEMPLATE_REQUIRED=NO')
print('ACCESS_TOKEN_STORED_ON_VPS=NO')
PY
RPY="/tmp/wfgg-radar-v4-context-$TAG.py"
...
ssh "${SSH_OPTS[@]}" "$REMOTE" "set -eu; test -s '$REMOTE_CAPTURE'; ! grep -q '^LASTWAR_NATIVE_SCAN_SEED=' '$REMOTE_ENV'; test -x '$REMOTE_BIN/radar-native-template'; test -x '$REMOTE_BIN/radar-connector'; grep -aFq 'native-template-readonly-v4' '$REMOTE_BIN/radar-native-template'; echo PLAYER_SCAN_V4=READY; echo STRATEGY=synthetic-world.get.block+get.user.info.multi; echo CAPTURED_MAP_TEMPLATE_REQUIRED=NO; echo ACCESS_TOKEN_ON_VPS=NO; echo SERVICE=\$(systemctl is-active wfgg-radar-connector)" </dev/null
say "=== PLAYER SCAN READONLY V4 : OK ==="
