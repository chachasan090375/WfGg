...
mkdir -p "$TMP_ROOT"
TMP="$(mktemp -d "$TMP_ROOT/wfgg-radar-v6198.XXXXXX")"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP="${DATA}/v6198-pilot-backups/${STAMP}"
SERVICE="wfgg-radar-connector"
PROMOTED=0

log(){ printf '%s\n' "$*"; }
fail(){ log "RADAR_V6198_PILOT_INSTALL=FAIL reason=$*"; exit 1; }
cleanup(){ rm -rf "$TMP"; }
rollback(){
  rc=$?
  if [[ "$rc" -ne 0 && "$PROMOTED" -eq 1 ]]; then
    log "RADAR_V6198_PILOT_ROLLBACK=START"
    ...
    sleep 2
    systemctl is-active --quiet "$SERVICE" && log "RADAR_V6198_PILOT_ROLLBACK=PASS" || log "RADAR_V6198_PILOT_ROLLBACK=FAIL"
  fi
  cleanup
  exit "$rc"
}
...
grep -Fq 'WFGG_RADAR_PILOT=V6.19.8' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_VERSION
grep -Fq 'COLLECTOR_CYCLE_TERMINALIZATION=YES' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_TERMINALIZATION
grep -Fq 'TERMINALIZATION_ATTEMPTS=12' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_RETRIES
...
install -m 0755 "$TMP/radar-connector" "$BIN/.radar-connector-v6198-new"
install -m 0755 "$TMP/radar-native-template" "$BIN/.radar-native-template-v6198-new"
mv -f "$BIN/.radar-connector-v6198-new" "$BIN/radar-connector"
mv -f "$BIN/.radar-native-template-v6198-new" "$BIN/radar-native-template"
PROMOTED=1
...
log "RADAR_V6198_PILOT_BACKUP=$BACKUP"
log "RADAR_V6198_LASTWAR_MUTATION=NO"
log "RADAR_V6198_PILOT_INSTALL=PASS"
