...
if ssh "${SSH_OPTS[@]}" -T "$REMOTE" 'true' </dev/null >/dev/null 2>&1; then
  echo FEDERATED_V63_INSPECT_SSH_ROUTE=ChaChaVPS
else
  SSH_OPTS+=(-o HostName="$PUBLIC_HOST")
  echo FEDERATED_V63_INSPECT_SSH_ROUTE=PUBLIC_IPV4
fi

ssh "${SSH_OPTS[@]}" -T "$REMOTE" '
echo "=== FEDERATED V6.3 SENTINEL ==="
journalctl -u wfgg-radar-connector --since "-15 min" --no-pager -o cat |
  grep "FEDERATED_COLLECTOR_V63_SENTINEL" |
  tail -n 120 || true

echo
echo "=== CONNECTOR HEALTH ==="
systemctl is-active wfgg-radar-connector || true
grep -aFq "FEDERATED_COLLECTOR_V63_SENTINEL" /opt/wfgg-radar/bin/radar-connector && echo FEDERATED_V63_CONNECTOR=READY || true
