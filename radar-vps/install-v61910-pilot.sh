...
REV="${WFGG_RADAR_V61910_REV:-radar-v61910-autopilot-continue-partial-limit}"
BASE="https://raw.githubusercontent.com/chachasan090375/WfGg/${REV}/radar-vps/pilot-v61910"
ROOT="/opt/wfgg-radar"
BIN="${ROOT}/bin"
DATA="${ROOT}/data"
TMP_ROOT="/opt/chacha-dev/runtime/tmp"
mkdir -p "$TMP_ROOT"
TMP="$(mktemp -d "$TMP_ROOT/wfgg-radar-v61910.XXXXXX")"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP="${DATA}/v61910-pilot-backups/${STAMP}"
SERVICE="wfgg-radar-connector"
PROMOTED=0

log(){ printf '%s\n' "$*"; }
fail(){ log "RADAR_V61910_PILOT_INSTALL=FAIL reason=$*"; exit 1; }
cleanup(){ rm -rf "$TMP"; }
rollback(){
  rc=$?
  if [[ "$rc" -ne 0 && "$PROMOTED" -eq 1 ]]; then
    log "RADAR_V61910_PILOT_ROLLBACK=START"
    ...
    sleep 2
    systemctl is-active --quiet "$SERVICE" && log "RADAR_V61910_PILOT_ROLLBACK=PASS" || log "RADAR_V61910_PILOT_ROLLBACK=FAIL"
  fi
  cleanup
  exit "$rc"
}
...
log "=== WFGG RADAR V6.19.10 AUTOPILOT CONTINUE-ON-PARTIAL-LIMIT PILOT INSTALL ==="
log "RADAR_V61910_REV=$REV"
...
grep -Fq 'WFGG_RADAR_PILOT=V6.19.10' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_VERSION
grep -Fq 'AUTOPILOT_CONTINUE_PARTIAL_LIMIT=YES' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_CONTINUE_PARTIAL
grep -Fq 'PARTIAL_LIMIT_GLOBAL_STOP=NO' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_PARTIAL_STOP
grep -Fq 'PARTIAL_VALID_EVIDENCE_PRESERVED=YES' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_PARTIAL_EVIDENCE
grep -Fq 'HISTORY_READONLY=YES' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_HISTORY_READONLY
grep -Fq 'STALE_CYCLE_RECOVERY_PRESERVED=YES' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_STALE
...
install -m 0755 "$TMP/radar-connector" "$BIN/.radar-connector-v61910-new"
install -m 0755 "$TMP/radar-native-template" "$BIN/.radar-native-template-v61910-new"
mv -f "$BIN/.radar-connector-v61910-new" "$BIN/radar-connector"
mv -f "$BIN/.radar-native-template-v61910-new" "$BIN/radar-native-template"
PROMOTED=1
...
log "RADAR_V61910_PILOT_BACKUP=$BACKUP"
log "RADAR_V61910_LASTWAR_MUTATION=NO"
log "RADAR_V61910_PILOT_INSTALL=PASS"
