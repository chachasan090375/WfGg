...
fail(){ printf 'RADAR_V6198_RUNTIME_PROBE=FAIL reason=%s\n' "$1"; exit 1; }
...
echo "RADAR_V6198_CONNECTOR_SHA=$ACTUAL_CONNECTOR"
echo "RADAR_V6198_NATIVE_SHA=$ACTUAL_NATIVE"
[[ "$ACTUAL_CONNECTOR" == "$EXPECTED_CONNECTOR" ]] || fail CONNECTOR_SHA_MISMATCH
[[ "$ACTUAL_NATIVE" == "$EXPECTED_NATIVE" ]] || fail NATIVE_SHA_MISMATCH

grep -aFq 'v6.19.8' "$BIN" || fail VERSION_MARKER_MISSING
grep -aFq 'COLLECTOR_CYCLE_TERMINALIZATION_FAILED' "$BIN" || fail TERMINALIZATION_MARKER_MISSING
...
echo "RADAR_V6198_TERMINALIZATION_MARKERS=PASS"
...
print("RADAR_V6198_START_ROUTE_GUARD=PASS")
