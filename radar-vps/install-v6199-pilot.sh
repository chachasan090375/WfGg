...
REV="${WFGG_RADAR_V6199_REV:-radar-v6199-autopilot-targeted-history-quality}"
BASE="https://raw.githubusercontent.com/chachasan090375/WfGg/${REV}/radar-vps/pilot-v6199"
ROOT="/opt/wfgg-radar"
BIN="${ROOT}/bin"
DATA="${ROOT}/data"
TMP_ROOT="/opt/chacha-dev/runtime/tmp"
mkdir -p "$TMP_ROOT"
TMP="$(mktemp -d "$TMP_ROOT/wfgg-radar-v6199.XXXXXX")"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP="${DATA}/v6199-pilot-backups/${STAMP}"
SERVICE="wfgg-radar-connector"
PROMOTED=0

log(){ printf '%s\n' "$*"; }
fail(){ log "RADAR_V6199_PILOT_INSTALL=FAIL reason=$*"; exit 1; }
cleanup(){ rm -rf "$TMP"; }
rollback(){
  rc=$?
  if [[ "$rc" -ne 0 && "$PROMOTED" -eq 1 ]]; then
    log "RADAR_V6199_PILOT_ROLLBACK=START"
    ...
    sleep 2
    systemctl is-active --quiet "$SERVICE" && log "RADAR_V6199_PILOT_ROLLBACK=PASS" || log "RADAR_V6199_PILOT_ROLLBACK=FAIL"
  fi
  cleanup
  exit "$rc"
}
...
log "=== WFGG RADAR V6.19.9 AUTOPILOT TARGETED HISTORY QUALITY PILOT INSTALL ==="
log "RADAR_V6199_REV=$REV"
...
grep -Fq 'WFGG_RADAR_PILOT=V6.19.9' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_VERSION
grep -Fq 'AUTOPILOT_TARGETED_HISTORY_QUALITY=YES' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_TARGETED_HISTORY
grep -Fq 'TARGETED_HISTORY_FULL_REBUILD=NO' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_FULL_REBUILD
...
install -m 0755 "$TMP/radar-connector" "$BIN/.radar-connector-v6199-new"
install -m 0755 "$TMP/radar-native-template" "$BIN/.radar-native-template-v6199-new"
mv -f "$BIN/.radar-connector-v6199-new" "$BIN/radar-connector"
mv -f "$BIN/.radar-native-template-v6199-new" "$BIN/radar-native-template"
PROMOTED=1
...
log "RADAR_V6199_PILOT_BACKUP=$BACKUP"
log "RADAR_V6199_LASTWAR_MUTATION=NO"
log "RADAR_V6199_PILOT_INSTALL=PASS"
