...
# Direct READONLY V4.2 native diagnostic from Termux.
# Streams the existing local session to the VPS over SSH, writes it only to a
# mode-0600 temporary file for the native process, then deletes it immediately.
# Output is deliberately restricted to safe scan counters and result metadata.
...
if len(str(d.get('accessToken','')).strip()) < 8:
    raise SystemExit(2)
if not str(d.get('gameUid','')).strip():
    raise SystemExit(3)
PY
...
HELPER="$TMP/wfgg-v42-native-diag.py"
cat > "$HELPER" <<'PY'
...
if not query:
    ...
    raise SystemExit(2)
if len(str(incoming.get('accessToken','')).strip()) < 8:
    print('ERROR=TOKEN_MISSING')
    raise SystemExit(2)
...
if not (os.path.isfile(native) and os.access(native,os.X_OK)):
    print('ERROR=NATIVE_BINARY_MISSING')
    raise SystemExit(2)
if not os.path.isfile(capture):
    print('ERROR=CAPTURE_MISSING')
    raise SystemExit(2)

fd,path=tempfile.mkstemp(prefix='wfgg-v42-session-',suffix='.json')
try:
    os.fchmod(fd,0o600)
    ...
    try:
        cp=subprocess.run(
            ...
            text=True,timeout=80,check=False,
        )
...
for line in cp.stderr.splitlines():
    if line.startswith('WFGG_SCAN_V42 '):
        print(line)
...
print('NATIVE_SCAN_PERFORMED='+str(d.get('scanPerformed',False)))
players=d.get('players')
if isinstance(players,list):
    print('NATIVE_PLAYERS='+str(len(players)))
    if players:
        p=players[0] if isinstance(players[0],dict) else {}
        safe={k:p.get(k) for k in ('pseudo','gameUid','serverId','allianceTag','x','y','hqLevel') if k in p}
        print('NATIVE_FIRST_PLAYER='+json.dumps(safe,ensure_ascii=False,separators=(',',':')))
elif players is None:
    print('NATIVE_PLAYERS=NULL')
else:
    print('NATIVE_PLAYERS_TYPE='+type(players).__name__)
PY

TAG="$$"
REMOTE_HELPER="/tmp/wfgg-v42-native-diag-$TAG.py"
