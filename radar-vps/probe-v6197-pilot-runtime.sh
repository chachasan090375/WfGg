...
EXPECTED_CONNECTOR="ccbb1a178212d3137136fff30c31725b88a7e3a8113dca922138018186a158d5"
EXPECTED_NATIVE="274d040f5294cb09422e5d55cc4b5335ac7739924c33dcb67b3f279645814900"

fail(){ printf 'RADAR_V6197_RUNTIME_PROBE=FAIL reason=%s\n' "$1"; exit 1; }
...
echo "RADAR_V6197_CONNECTOR_SHA=$ACTUAL_CONNECTOR"
echo "RADAR_V6197_NATIVE_SHA=$ACTUAL_NATIVE"
[[ "$ACTUAL_CONNECTOR" == "$EXPECTED_CONNECTOR" ]] || fail CONNECTOR_SHA_MISMATCH
[[ "$ACTUAL_NATIVE" == "$EXPECTED_NATIVE" ]] || fail NATIVE_SHA_MISMATCH

grep -aFq 'v6.19.7' "$BIN" || fail VERSION_MARKER_MISSING
grep -aFq 'STALE_RUNNING_CYCLE_RECOVERED_V6197' "$BIN" || fail STALE_RECOVERY_MARKER_MISSING
...
echo "RADAR_V6197_RECOVERY_MARKERS=PASS"
...
print("RADAR_V6197_START_ROUTE_GUARD=PASS")
...
print("RADAR_V6197_STATUS_ROUTE_GUARD=PASS")

status,payload=request("POST","/v1/collector/autopilot/stop",{"id":"does-not-exist"})
assert status==404,(status,payload)
assert payload.get("error")=="AUTOPILOT_JOB_NOT_FOUND",payload
print("RADAR_V6197_STOP_ROUTE_GUARD=PASS")
PY

echo "RADAR_V6197_GAME_SCAN_EXECUTED=NO"
echo "RADAR_V6197_COLLECTOR_MUTATION=NO"
echo "RADAR_V6197_RUNTIME_PROBE=PASS"
