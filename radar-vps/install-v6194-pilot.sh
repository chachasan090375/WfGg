...
REV="${WFGG_RADAR_PILOT_REV:-${WFGG_RADAR_V6191_REV:-${WFGG_RADAR_V6194_REV:-radar-v6194-autopilot}}}"
BASE="https://raw.githubusercontent.com/chachasan090375/WfGg/${REV}/radar-vps/pilot-v6194"
ROOT="/opt/wfgg-radar"
BIN="${ROOT}/bin"
DATA="${ROOT}/data"
SERVICE="wfgg-radar-connector"
TMP="$(mktemp -d /tmp/wfgg-radar-v6194.XXXXXX)"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP="${DATA}/v6194-pilot-backups/${STAMP}"
PROMOTED=0

log(){ printf '%s\n' "$*"; }
fail(){ log "RADAR_V6194_PILOT_INSTALL=FAIL reason=$*"; exit 1; }
cleanup(){ rm -rf "$TMP"; }
rollback(){
  rc=$?
  if [[ "$rc" -ne 0 && "$PROMOTED" -eq 1 ]]; then
    log "RADAR_V6194_PILOT_ROLLBACK=START"
    ...
    sleep 2
    systemctl is-active --quiet "$SERVICE" && log "RADAR_V6194_PILOT_ROLLBACK=PASS" || log "RADAR_V6194_PILOT_ROLLBACK=FAIL"
  fi
  cleanup
  exit "$rc"
}
...
log "=== WFGG RADAR V6.19.4 AUTOPILOT PILOT INSTALL ==="
log "RADAR_V6194_REV=$REV"
...
grep -Fq 'WFGG_RADAR_PILOT=V6.19.4' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_VERSION
...
install -m 0755 "$TMP/radar-connector" "$BIN/.radar-connector-v6194-new"
install -m 0755 "$TMP/radar-native-template" "$BIN/.radar-native-template-v6194-new"
mv -f "$BIN/.radar-connector-v6194-new" "$BIN/radar-connector"
mv -f "$BIN/.radar-native-template-v6194-new" "$BIN/radar-native-template"
PROMOTED=1
...
log "RADAR_V6194_PILOT_BACKUP=$BACKUP"
log "RADAR_V6194_PRODUCTION_MUTATION=NO"
log "RADAR_V6194_PILOT_INSTALL=PASS"
