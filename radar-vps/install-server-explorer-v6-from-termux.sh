...
RELEASE_COMMIT="f4156947e1ed374e94b61c36cc07441bcb50917a"
...
if ssh "${SSH_OPTS[@]}" -T "$REMOTE" 'true' </dev/null >/dev/null 2>&1; then
  say 'SERVER_EXPLORER_V6_SSH_ROUTE=ChaChaVPS'
else
  SSH_OPTS+=(-o HostName="$PUBLIC_HOST")
  ssh "${SSH_OPTS[@]}" -T "$REMOTE" 'true' </dev/null >/dev/null 2>&1 || die VPS_UNREACHABLE
  say 'SERVER_EXPLORER_V6_SSH_ROUTE=PUBLIC_IPV4'
fi

say '=== WfGg Radar · Server Explorer V6 · single-session batch ==='
...
say 'SERVER_EXPLORER_V6_RELEASE=OK'

say '2/4 Sauvegarde atomique des binaires actuels'
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP_CONNECTOR="$BACKUP_DIR/radar-connector-before-server-explorer-v6-$STAMP"
BACKUP_NATIVE="$BACKUP_DIR/radar-native-template-before-server-explorer-v6-$STAMP"
...
echo SERVER_EXPLORER_V6_BACKUP_CONNECTOR='$BACKUP_CONNECTOR'
echo SERVER_EXPLORER_V6_BACKUP_NATIVE='$BACKUP_NATIVE'
" </dev/null

say '3/4 Transfert SSH direct puis installation avec rollback automatique'
RCON="/tmp/wfgg-server-explorer-v6-connector-$$"
RNAT="/tmp/wfgg-server-explorer-v6-native-$$"
cat "$TMP/radar-connector" | ssh "${SSH_OPTS[@]}" -T "$REMOTE" "cat > '$RCON'"
cat "$TMP/radar-native-template" | ssh "${SSH_OPTS[@]}" -T "$REMOTE" "cat > '$RNAT'"
ssh "${SSH_OPTS[@]}" -T "$REMOTE" "
set -eu
rollback(){
  echo SERVER_EXPLORER_V6_ROLLBACK=START
  ...
  echo SERVER_EXPLORER_V6_ROLLBACK=DONE
}
...
echo SERVER_EXPLORER_V6_SERVICE=\$(systemctl is-active wfgg-radar-connector.service)
" </dev/null
...
echo SERVER_EXPLORER_V6_CONNECTOR=READY
echo SERVER_EXPLORER_V6_NATIVE_BATCH=READY
echo SERVER_EXPLORER_V5_FALLBACK=PRESERVED
echo BROAD_SCAN_V4_FALLBACK=PRESERVED
echo SERVER_EXPLORER_V6_CONTROL_BATCH=989-995
echo RADAR_PRODUCTION_CHANGED=NO
" </dev/null

say 'SERVER_EXPLORER_V6=OK'
