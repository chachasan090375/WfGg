...
REV="${WFGG_RADAR_V6196_REV:-radar-v6196-autopilot-continue-no-data}"
BASE="https://raw.githubusercontent.com/chachasan090375/WfGg/${REV}/radar-vps/pilot-v6196"
ROOT="/opt/wfgg-radar"
BIN="${ROOT}/bin"
DATA="${ROOT}/data"
TMP_ROOT="/opt/chacha-dev/runtime/tmp"
mkdir -p "$TMP_ROOT"
TMP="$(mktemp -d "$TMP_ROOT/wfgg-radar-v6196.XXXXXX")"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP="${DATA}/v6196-pilot-backups/${STAMP}"
SERVICE="wfgg-radar-connector"
PROMOTED=0

log(){ printf '%s\n' "$*"; }
fail(){ log "RADAR_V6196_PILOT_INSTALL=FAIL reason=$*"; exit 1; }
cleanup(){ rm -rf "$TMP"; }
rollback(){
  rc=$?
  if [[ "$rc" -ne 0 && "$PROMOTED" -eq 1 ]]; then
    log "RADAR_V6196_PILOT_ROLLBACK=START"
    ...
    sleep 2
    systemctl is-active --quiet "$SERVICE" && log "RADAR_V6196_PILOT_ROLLBACK=PASS" || log "RADAR_V6196_PILOT_ROLLBACK=FAIL"
  fi
  cleanup
  exit "$rc"
}
...
for c in curl sha256sum install systemctl mktemp cp mv date awk; do
  command -v "$c" >/dev/null 2>&1 || fail "COMMAND_MISSING:$c"
done
systemctl is-active --quiet "$SERVICE" || fail SERVICE_NOT_ACTIVE

log "=== WFGG RADAR V6.19.6 CONTINUE-ON-NO-DATA PILOT INSTALL ==="
log "RADAR_V6196_REV=$REV"
...
grep -Fq 'WFGG_RADAR_PILOT=V6.19.6' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_VERSION
grep -Fq 'NO_DATA_RESULT=SKIPPED_NOT_CONFIRMED' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_NO_DATA
grep -Fq 'VALID_EVIDENCE_PRESERVED=YES' "$TMP/PILOT_INFO.txt" || fail PILOT_INFO_EVIDENCE
...
install -m 0755 "$TMP/radar-connector" "$BIN/.radar-connector-v6196-new"
install -m 0755 "$TMP/radar-native-template" "$BIN/.radar-native-template-v6196-new"
mv -f "$BIN/.radar-connector-v6196-new" "$BIN/radar-connector"
mv -f "$BIN/.radar-native-template-v6196-new" "$BIN/radar-native-template"
PROMOTED=1
...
log "RADAR_V6196_PILOT_BACKUP=$BACKUP"
log "RADAR_V6196_LASTWAR_MUTATION=NO"
log "RADAR_V6196_PILOT_INSTALL=PASS"
