...
RELEASE_COMMIT="c031ef6f37e58054b616b2c5e27901745ba07949"
...
if ssh "${SSH_OPTS[@]}" -T "$REMOTE" 'true' </dev/null >/dev/null 2>&1; then
  say 'FEDERATED_V631_SSH_ROUTE=ChaChaVPS'
else
  SSH_OPTS+=(-o HostName="$PUBLIC_HOST")
  ssh "${SSH_OPTS[@]}" -T "$REMOTE" 'true' </dev/null >/dev/null 2>&1 || die VPS_UNREACHABLE
  say 'FEDERATED_V631_SSH_ROUTE=PUBLIC_IPV4'
fi

say '=== WfGg Radar · Federated Collector V6.3.1 · topology sentinel ==='
...
grep -aFq 'FEDERATED_COLLECTOR_V631_TOPOLOGY' "$TMP/radar-connector" || die FEDERATED_V631_TOPOLOGY_MARKER_MISSING
...
say 'FEDERATED_V631_RELEASE=OK'

say '2/4 Sauvegarde atomique des binaires actuels'
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP_CONNECTOR="$BACKUP_DIR/radar-connector-before-federated-v631-$STAMP"
BACKUP_NATIVE="$BACKUP_DIR/radar-native-template-before-federated-v631-$STAMP"
...
echo FEDERATED_V631_BACKUP_CONNECTOR='$BACKUP_CONNECTOR'
echo FEDERATED_V631_BACKUP_NATIVE='$BACKUP_NATIVE'
" </dev/null

say '3/4 Transfert SSH direct puis installation avec rollback automatique'
RCON="/tmp/wfgg-federated-v631-connector-$$"
RNAT="/tmp/wfgg-federated-v631-native-$$"
cat "$TMP/radar-connector" | ssh "${SSH_OPTS[@]}" -T "$REMOTE" "cat > '$RCON'"
cat "$TMP/radar-native-template" | ssh "${SSH_OPTS[@]}" -T "$REMOTE" "cat > '$RNAT'"
ssh "${SSH_OPTS[@]}" -T "$REMOTE" "
set -eu
rollback(){
  echo FEDERATED_V631_ROLLBACK=START
  ...
  echo FEDERATED_V631_ROLLBACK=DONE
}
...
echo FEDERATED_V631_SERVICE=\$(systemctl is-active wfgg-radar-connector.service)
" </dev/null
