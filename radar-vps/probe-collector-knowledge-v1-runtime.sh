...
curl -fsS "http://127.0.0.1:8791/healthz" >/tmp/chacha-learning-ingress-health.json
...
ci=json.load(open('/tmp/chacha-learning-ingress-health.json'))
...
assert ci.get('status')=='ok',ci
assert ci.get('service')=='chacha-central-learning-ingress',ci
