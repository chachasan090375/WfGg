...
RELEASE_COMMIT="e0ebd2a68c7b47b48ff90138ca2e916c5d310a9f"
...
if ssh "${SSH_OPTS[@]}" -T "$REMOTE" 'true' </dev/null >/dev/null 2>&1; then
  say 'SERVER_EXPLORER_V61_SSH_ROUTE=ChaChaVPS'
else
  SSH_OPTS+=(-o HostName="$PUBLIC_HOST")
  ssh "${SSH_OPTS[@]}" -T "$REMOTE" 'true' </dev/null >/dev/null 2>&1 || die VPS_UNREACHABLE
  say 'SERVER_EXPLORER_V61_SSH_ROUTE=PUBLIC_IPV4'
fi

say '=== WfGg Radar · Server Explorer V6.1 · discovery ==='
...
grep -aFq 'SERVER_EXPLORER_V6_SINGLE_SESSION' "$TMP/radar-connector" || die V6_CONNECTOR_MARKER_MISSING
grep -aFq 'SERVER_DISCOVERY_V61_SENTINEL' "$TMP/radar-connector" || die V61_DISCOVERY_MARKER_MISSING
grep -aFq 'LASTWAR_SERVER_BATCH_EMPTY' "$TMP/radar-connector" || die V6_BATCH_ADAPTER_MISSING
grep -aFq 'SERVER_EXPLORER_NATIVE_SENTINEL' "$TMP/radar-native-template" || die NATIVE_SENTINEL_MARKER_MISSING
grep -aFq 'world.get.block(batch-v6)' "$TMP/radar-native-template" || die V6_NATIVE_BATCH_MARKER_MISSING
grep -aFq 'LOGIN_SERVERINFO_REDIRECT_PRESENT' "$TMP/radar-native-template" || die V5_REDIRECT_FALLBACK_MISSING
grep -aFq 'native-template-readonly-v4' "$TMP/radar-native-template" || die BROAD_SCAN_V4_MARKER_MISSING
say 'SERVER_EXPLORER_V61_RELEASE=OK'

say '2/4 Sauvegarde atomique des binaires actuels'
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP_CONNECTOR="$BACKUP_DIR/radar-connector-before-server-explorer-v61-$STAMP"
BACKUP_NATIVE="$BACKUP_DIR/radar-native-template-before-server-explorer-v61-$STAMP"
...
echo SERVER_EXPLORER_V61_BACKUP_CONNECTOR='$BACKUP_CONNECTOR'
echo SERVER_EXPLORER_V61_BACKUP_NATIVE='$BACKUP_NATIVE'
" </dev/null

say '3/4 Transfert SSH direct puis installation avec rollback automatique'
RCON="/tmp/wfgg-server-explorer-v61-connector-$$"
RNAT="/tmp/wfgg-server-explorer-v61-native-$$"
cat "$TMP/radar-connector" | ssh "${SSH_OPTS[@]}" -T "$REMOTE" "cat > '$RCON'"
cat "$TMP/radar-native-template" | ssh "${SSH_OPTS[@]}" -T "$REMOTE" "cat > '$RNAT'"
ssh "${SSH_OPTS[@]}" -T "$REMOTE" "
set -eu
rollback(){
  echo SERVER_EXPLORER_V61_ROLLBACK=START
  ...
  echo SERVER_EXPLORER_V61_ROLLBACK=DONE
}
...
grep -aFq 'SERVER_DISCOVERY_V61_SENTINEL' '$RADAR_CONNECTOR'
grep -aFq 'world.get.block(batch-v6)' '$RADAR_NATIVE'
trap - HUP INT TERM ERR
echo SERVER_EXPLORER_V61_SERVICE=\$(systemctl is-active wfgg-radar-connector.service)
" </dev/null
...
grep -aFq 'SERVER_EXPLORER_V6_SINGLE_SESSION' '$RADAR_CONNECTOR'
grep -aFq 'SERVER_DISCOVERY_V61_SENTINEL' '$RADAR_CONNECTOR'
grep -aFq 'LASTWAR_SERVER_BATCH_EMPTY' '$RADAR_CONNECTOR'
grep -aFq 'SERVER_EXPLORER_NATIVE_SENTINEL' '$RADAR_NATIVE'
grep -aFq 'world.get.block(batch-v6)' '$RADAR_NATIVE'
grep -aFq 'LOGIN_SERVERINFO_REDIRECT_PRESENT' '$RADAR_NATIVE'
grep -aFq 'native-template-readonly-v4' '$RADAR_NATIVE'
echo SERVER_EXPLORER_V61_DISCOVERY=READY
echo SERVER_EXPLORER_V6_SINGLE_SESSION=READY
echo SERVER_EXPLORER_V5_FALLBACK=PRESERVED
echo BROAD_SCAN_V4_FALLBACK=PRESERVED
echo SERVER_EXPLORER_V61_CONTROL=@discover:992
echo RADAR_PRODUCTION_CHANGED=NO
" </dev/null

say 'SERVER_EXPLORER_V61=OK'
