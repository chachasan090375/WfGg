(() => {
  ...
  window.WfGgHeroNativeAnimationV17C={version:'17.3.0-master-linked'};
  ...
  function applyCrop(card){
    const p=window.WfGgHeroPortraitV18?.profileFor?.(card.dataset.heroId);
    if(!p)return;
  ...
  function stop({restore=true}={}){
    ...
    if(restore){active.forEach(toStill);active=[]}
    document.documentElement.dataset.wfggV17cPhase='idle';
  ...
  function preload(url,token){
    return new Promise(resolve=>{
      ...
      if(img.complete)finish(true); setTimeout(()=>finish(false),2500);
    });
  }
  ...
  async function activateNext(token){
    ...
    if(!candidates.length){timer=setTimeout(()=>activateNext(token),1200);return}
    ...
    for(const card of active){
      neutralize(card); applyCrop(card);
      const a=info(card.dataset.heroId); const img=imgFor(card);
      if(a?.src&&img){img.src=a.src;card.classList.add('wfgg-native-gif-active-v17c');card.dataset.wfggNativeAnimation='native-gif-playing-v17c'}
    }
  ...
  function start(){
    ...
    viewportCards().forEach(card=>{neutralize(card);if(info(card.dataset.heroId))toStill(card)});
    const launch=()=>{if(token!==generation||!running||!stepVisible())return;activateNext(token)};
    if('requestIdleCallback' in window)requestIdleCallback(launch,{timeout:START_DELAY_MS+1000});
    else timer=setTimeout(launch,START_DELAY_MS);
  }

  function syncVisibility(){
    if(stepVisible()){
      if(!running){clearTimeout(timer);timer=setTimeout(start,START_DELAY_MS)}
    }else stop({restore:true});
  }
  ...
  async function init(){
    ...
    const g=grid(); if(g){
      gridObserver=new MutationObserver(()=>{
        if(stepVisible())viewportCards().forEach(card=>{neutralize(card);if(info(card.dataset.heroId)&&!card.classList.contains('wfgg-native-gif-active-v17c'))toStill(card)})
      });
    ...
    window.addEventListener('scroll',()=>{
      ...
      clearTimeout(timer);
      timer=setTimeout(()=>{if(stepVisible()){active.forEach(toStill);active=[];activateNext(generation)}},450);
    },{passive:true});
    syncVisibility();
  }
