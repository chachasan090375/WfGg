...
export async function routeLastWarIdentity(request, env, url, deps) {
  ...
  if (url.pathname === '/api/lastwar/health') {
    ...
    const base = cleanBrokerUrl(env.LASTWAR_BROKER_URL);
    if (!base) return json({ ok: false, error: 'LASTWAR_BROKER_NOT_CONFIGURED' }, 503);
    try {
      ...
      if (!response.ok || data?.ok !== true || data?.service !== 'wfgg-lastwar-broker-manager') {
        return json({ ok: false, error: 'LASTWAR_BROKER_HEALTH_FAILED' }, 503);
      }
      return json({
        ok: true,
        service: 'wfgg-lastwar-connector',
        broker: 'wfgg-lastwar-broker-manager',
        child_broker: data.child_service || null,
        mode: data.mode === 'read-only' ? 'read-only' : 'unknown',
        revision: BROKER_REVISION.slice(0, 8),
        external: true,
        slots: clampInt(data.slots, 1, 32, 1),
        healthy_slots: clampInt(data.healthy_slots, 0, 32, 0),
        leased_slots: clampInt(data.leased_slots, 0, 32, 0)
      });
    } catch (_) {
  ...
  if (url.pathname === '/api/lastwar/identity/resolve') {
    ...
    const broker = await brokerCall(env, ctx, '/v1/identity/resolve', {
      user_id: ctx.id,
      uid,
  ...
  if (url.pathname === '/api/lastwar/identity/send-code') {
    ...
    const broker = await brokerCall(env, ctx, '/v1/identity/send-code', {
      user_id: ctx.id,
      uid,
      email,
      locale: cleanLocale(ctx.language)
    ...
    rejectSensitiveBrokerFields(broker, fail);
    const authTransaction = cleanOpaque(broker.auth_transaction, 200);
  ...
  if (url.pathname === '/api/lastwar/identity/verify-code') {
    ...
    const authTransaction = cleanOpaque(body.auth_transaction, 200);
    if (!uid) fail('LASTWAR_UID_INVALID', 400);
    if (!CODE_RE.test(code)) fail('LASTWAR_VERIFY_CODE_INVALID', 400);
    if (!authTransaction) fail('LASTWAR_AUTH_TRANSACTION_REQUIRED', 400);

    const broker = await brokerCall(env, ctx, '/v1/identity/verify-code', {
      user_id: ctx.id,
      uid,
      code,
      auth_transaction: authTransaction,
    ...
    const cloudLinkHash = await sha256Text(`wfgg-cloud-link:v2:${ctx.id}:${uid}`);
    ...
    try {
      ...
      `).bind(
        ...
        cloudLinkHash,
        'WfGg Cloud Last War',
        `external-${BROKER_REVISION.slice(0, 8)}`,
        uid,
        ts,
      ...
      console.error('lastwar external link persistence failed', err?.message || err);
...
export async function routeLastWarCloudSync(request, env, url, deps) {
  ...
  const ctx = await sessionContext(request, env);
  const broker = await brokerCall(env, ctx, '/v1/profile/sync', {
    user_id: ctx.id,
    locale: cleanLocale(ctx.language)
  }, sha256Text, fail);
...
async function brokerCall(env, ctx, path, payload, sha256Text, fail) {
  const base = cleanBrokerUrl(env.LASTWAR_BROKER_URL);
  const authSecret = cleanText(env.LASTWAR_BROKER_SHARED_SECRET, 512);
  if (!base || authSecret.length < 32) fail('LASTWAR_BROKER_NOT_CONFIGURED', 503);
  ...
  const stored = await env.DB.prepare(
    'SELECT sealed_state FROM lastwar_cloud_state WHERE user_id=? LIMIT 1'
  ).bind(userId).first();

  const headers = {
    'Content-Type': 'application/json',
    'X-WfGg-Broker-Auth': authSecret,
    'X-WfGg-State-Key': stateKey,
    'Cache-Control': 'no-store'
  };
  if (stored?.sealed_state) headers['X-WfGg-Sealed-State'] = String(stored.sealed_state);
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    const sealed = typeof data._wfgg_sealed_state === 'string'
      ? data._wfgg_sealed_state.trim()
      : '';
    if (sealed) {
      if (sealed.length > 16_384 || !sealed.startsWith('wfgs1.')) {
        fail('LASTWAR_BROKER_STATE_INVALID', 502);
      }
      const sourceUid = normalizeUid(data.uid || payload.uid) || null;
      await env.DB.prepare(`
        INSERT INTO lastwar_cloud_state(user_id,source_uid,sealed_state,updated_at)
        VALUES(?,?,?,?)
        ON CONFLICT(user_id) DO UPDATE SET
          source_uid=excluded.source_uid,
          sealed_state=excluded.sealed_state,
          updated_at=excluded.updated_at
      `).bind(userId, sourceUid, sealed, new Date().toISOString()).run();
      delete data._wfgg_sealed_state;
    }
  }
...
async function ensureStateTable(env) {
  ...
  await env.DB.prepare(`
    CREATE TABLE IF NOT EXISTS lastwar_cloud_state (
...
function cleanBrokerUrl(value) {
  const s = cleanText(value, 500).replace(/\/+$/, '');
  if (!/^https:\/\/[A-Za-z0-9.-]+(?::\d+)?$/.test(s)) return '';
  return s;
}
