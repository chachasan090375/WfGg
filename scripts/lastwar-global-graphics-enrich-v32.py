...
def load_events():
    idx=json.loads(REGISTRY.read_text('utf-8'))
    out=[]
    for spec in idx.get('files',[]):
        ...
        for e in data.get('events',[]):
            toks=[]
            for t in e.get('assetTokens',[]):
                nd=dense(t)
                # Very short/generic tokens create destructive false positives.
                if len(nd)>=5: toks.append(nd)
            if not toks: continue
            out.append({
                'id':e['id'],'name':e.get('name',e['id']),'kind':e.get('kind','event'),
                'phase':e.get('phase',''),'category':e.get('category',''),'cadence':e.get('cadence',''),
                'tokens':sorted(set(toks),key=len,reverse=True),'confidence':e.get('confidence','high')
            })
    return out
...
def ensure_columns(con):
    ...
    additions={
      'graphic_class':'TEXT','graphic_conf':'REAL','graphic_evidence_json':'TEXT','is_graphic':'INTEGER'
    }
...
def main():
    ...
    ensure_columns(con)
    events=load_events()
    con.executemany('INSERT INTO event_registry_v32 VALUES(?,?,?,?,?,?,?,?)',[
    ...
    for row in rows:
        ...
        hay=dense(' '.join(str(row[k] or '') for k in ('asset_path','logical_name','alias_name')))
        matched=[]
        for e in events:
            hits=[t for t in e['tokens'] if t in hay]
            if not hits: continue
            # Curated direct token inside the asset/bundle naming means intrinsic event ownership.
    ...
    rebuild_facets(con)
    con.commit()
    linked=con.execute('SELECT count(*) FROM event_asset_links_v32').fetchone()[0]
    linked_assets=con.execute('SELECT count(distinct stable_id) FROM event_asset_links_v32').fetchone()[0]
    con.close()
