...
// FederatedRegionDiagnosticScanner is the V6.15 read-only extension that lets
...
func normalizeFederatedTargetServerV615(serverID string) (string, error) {
...
func (c *NativeTemplateReadonly) ScanPlayerRegionDiagnosticOnServer(parent context.Context, token, query string, region int, serverID string) ([]Player, RegionScanDiagnostic, error) {
	...
	target, err := normalizeFederatedTargetServerV615(serverID)
	if err != nil {
		return nil, diag, err
	}
	...
	dir, err := os.MkdirTemp("", "wfgg-radar-v615-region-*")
	if err != nil {
