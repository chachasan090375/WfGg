...
REV="${WFGG_DEV_HUB_RADAR_V6199_AUTOPILOT_ACTIVATION_REV:-}"
PROJECT="wfgg-radar"
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v6199-autopilot-activate.XXXXXX")"
...
die(){ echo "RADAR_V6199_AUTOPILOT_ACTIVATION=BLOCKED reason=$1"; exit 2; }
cleanup(){ rm -rf "$WORK"; }
...
echo "=== CHACHA DEV RADAR V6.19.9 AUTOPILOT PRODUCTION ACTIVATION ==="
echo "SOURCE_REV=$REV"
...
echo "RADAR_V6199_AUTOPILOT_ACTIVATION_CONTRACT_TESTS=PASS"

WFGG_DEV_HUB_RADAR_ADAPTER_REV="$REV" bash dev-hub/bin/install-radar-runtime-adapter-pilot.sh >/dev/null
echo "RADAR_V6199_AUTOPILOT_ACTIVATION_ADAPTER=READY"
...
# Fail closed unless the exact V6.19.9 production runtime and Cloudflare surface are live.
...
print('RADAR_V6199_AUTOPILOT_CLOUDFLARE_HEALTH=PASS')
PY
...
echo "RADAR_V6199_AUTOPILOT_PRODUCTION_PREFLIGHT=PASS"
...
echo "RADAR_V6199_AUTOPILOT_BASELINE_CYCLE_ID=$BASELINE_ID"
echo "RADAR_V6199_AUTOPILOT_BASELINE_QUIET=PASS"

revoke_approval () {
  [ "$APPROVAL_ACTIVE" -eq 1 ] || return 0
  local ev="$EVIDENCE_DIR/approval-radar-v6199-autopilot-production-activation-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6199-autopilot-production-activation-revoked.task-graph.json"
...
obj={
 ...
 "statement":"The one-shot V6.19.9 Autopilot production activation approval has been consumed or the activation window has ended.",
...
graph={"schema":"chacha.dev/task-graph/v1","project":"wfgg-radar","transition":"AUTOPILOT-ACTIVATION-APPROVAL-REVOKE","generated_at":now,
 "tasks":[{"id":"approval:radar-v6199-autopilot-activation-revoke","kind":"approval",
 "description":"Revoke consumed V6.19.9 Autopilot production activation approval.","owner_role":"project-owner",
...
res={"schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6199-autopilot-activation-revoke",
...
PY
  then
    echo "RADAR_V6199_AUTOPILOT_ACTIVATION_APPROVAL_REVOKED=PASS"
    APPROVAL_ACTIVE=0
  else
    echo "RADAR_V6199_AUTOPILOT_ACTIVATION_APPROVAL_REVOKED=FAIL"
  fi
}
...
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6199-autopilot-production-activation.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6199-autopilot-production-activation.task-graph.json"
...
obj={
 ...
 "scope":{"release":"V6.19.9 Autopilot Targeted History Quality","action":"activate production Autopilot",
 ...
 "statement":"J’approuve l’activation production de Radar Autopilot V6.19.9 en mode READ-ONLY Last War, avec mutations Collector limitées aux cycles de découverte, preuves et terminalisation.",
...
graph={"schema":"chacha.dev/task-graph/v1","project":"wfgg-radar","transition":"AUTOPILOT-PRODUCTION-ACTIVATION-APPROVAL","generated_at":now,
 "tasks":[{"id":"approval:radar-v6199-autopilot-activation","kind":"approval",
 "description":"Record explicit human approval for V6.19.9 production Autopilot activation.","owner_role":"project-owner",
 "capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"radar-autopilot-production-activation"}],
 "verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],
 "summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={"schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6199-autopilot-activation",
 "status":"OK","producer":"human-approval-recorder","observed_at":now,"summary":"Human approved V6.19.9 production Autopilot activation.",
 "evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"radar-autopilot-production-activation","scope":"v6199-readonly-autopilot"}}],
...
print('RADAR_V6199_AUTOPILOT_ACTIVATION_APPROVAL_INGESTED=PASS')
PY
APPROVAL_ACTIVE=1

echo "RADAR_V6199_AUTOPILOT_ACTIVATION_WINDOW_READY=YES"
echo "RADAR_V6199_AUTOPILOT_ACTIVATION_ACTION=Dans Radar production, appuie UNE SEULE FOIS sur DÉMARRER L’AUTOPILOT. Ne lance aucune recherche manuelle."
echo "RADAR_V6199_AUTOPILOT_ACTIVATION_WAIT_MAX_SECONDS=1200"
echo "RADAR_V6199_LASTWAR_MODE=READ_ONLY"
echo "RADAR_V6199_TOKEN_CAPTURE=NO"
echo "RADAR_V6199_TOKEN_PERSISTENCE=NO"
...
while time.time()<deadline:
    ...
    if row:
        ...
        if state!=last:
            print("RADAR_V6199_AUTOPILOT_FIRST_CYCLE_ID="+str(row['id']),flush=True)
            print("RADAR_V6199_AUTOPILOT_FIRST_QUERY="+str(row['query']),flush=True)
            print("RADAR_V6199_AUTOPILOT_FIRST_STATUS="+str(row['status']),flush=True)
            print("RADAR_V6199_AUTOPILOT_FIRST_ERROR="+(str(row['error']) or 'NONE'),flush=True)
            last=state
        print("RADAR_V6199_AUTOPILOT_ACTIVITY_OBSERVED=PASS",flush=True)
        raise SystemExit(0)
    time.sleep(5)
print("RADAR_V6199_AUTOPILOT_ACTIVATION_FAIL=NO_NEW_TARGETED_CYCLE_WITHIN_1200S",flush=True)
raise SystemExit(3)
PY
ACTIVATION_RC=$?
set -e
...
echo "RADAR_V6199_AUTOPILOT_ACTIVATION_APPROVAL_STANDING=NO"
echo "RADAR_V6199_AUTOPILOT_PRODUCTION=ACTIVE_OBSERVED"
echo "RADAR_V6199_AUTOPILOT_ACTIVATION=PASS"
echo "RADAR_V6199_AUTOPILOT_CHACHA_PATH=PROJECT_CONTROL>HUMAN_APPROVAL>BROWSER_START>COLLECTOR_EVIDENCE"
