...
PROJECT="wfgg-radar"
EXPECTED_CONNECTOR="73b64743031e4a47212dfdafae8ad67551f88d131831e70cb08a40957824f9a8"
...
WORK="$(mktemp -d /tmp/chacha-radar-v6193-prod-reconcile.XXXXXX)"
...
cleanup(){ rm -rf "$WORK"; }
trap cleanup EXIT
die(){ echo "RADAR_V6193_PROD_RECONCILE=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.3 PRODUCTION SENTINEL RECONCILE ==="
echo "SOURCE_REV=$REV"
...
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6193-production-promotion.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6193-production-promotion.task-graph.json"
...
obj={
  ...
  "project":"wfgg-radar",
  "scope":{
    "release":"V6.19.3 Seed Scout",
    "action":"production promotion and associated Cloudflare surfaces",
    "connector_sha256":"73b64743031e4a47212dfdafae8ad67551f88d131831e70cb08a40957824f9a8",
    "native_sha256":"274d040f5294cb09422e5d55cc4b5335ac7739924c33dcb67b3f279645814900"
  },
  "actor":"human-user",
  "source":"chat-explicit-approval",
  "statement":"J’approuve la promotion production Radar V6.19.3 Seed Scout et le déploiement des surfaces Cloudflare associées.",
...
"tasks":[{"id":"approval:radar-v6193-production","kind":"approval","description":"Record explicit human approval for V6.19.3 production promotion.","owner_role":"project-owner","capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"production-release"}],"verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],
"summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={"schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6193-production","status":"OK","producer":"human-approval-recorder","observed_at":now,"summary":"Human approved V6.19.3 production promotion.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","scope":"radar-v6193-production"}}],"outputs":[{"type":"approval","id":"production-release","status":"APPROVED"}],"verification":{"status":"UNVERIFIED","method":"none","verifier":None,"observed_at":None,"notes":"Human verification required."}}
...
GRAPH="$PLAN_DIR/radar-v6193-production-sentinel-reconcile.task-graph.json"
python3 - "$GRAPH" <<'PY'
import json,sys
from datetime import datetime,timezone
...
"tasks":[{"id":"radar-runtime:production-sentinel-reconcile-v6193","kind":"runtime-deploy","description":"Trigger Radar Sentinel reconciliation against the already published V6.19.3 production release and verify the resulting runtime SHA.","owner_role":"sre-observability","capabilities":["radar-pilot-control"],"permission":"production-deploy","depends_on":[],"outputs":[{"type":"gate","id":"radar-v6193-production-runtime"}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-close"}}}],
...
graph={"schema":"chacha.dev/task-graph/v1","project":"wfgg-radar","transition":"PRODUCTION-APPROVAL-REVOKE","generated_at":now,"tasks":[{"id":"approval:radar-v6193-production-revoke","kind":"approval","description":"Revoke consumed V6.19.3 production approval.","owner_role":"project-owner","capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"production-release"}],"verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],"summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={"schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6193-production-revoke","status":"OK","producer":"human-approval-recorder","observed_at":now,"summary":"Consumed V6.19.3 production approval revoked.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","status":"REJECTED"}}],"outputs":[{"type":"approval","id":"production-release","status":"REJECTED"}],"verification":{"status":"UNVERIFIED","method":"none","verifier":None,"observed_at":None,"notes":"Human verification required."}}
open(g,"w",encoding="utf-8").write(json.dumps(graph,indent=2)+"\n"); open(r,"w",encoding="utf-8").write(json.dumps(res,indent=2)+"\n")
PY
...
echo "RADAR_V6193_CHACHA_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER>SENTINEL"
echo "RADAR_V6193_PROD_RECONCILE=PASS"
