...
REV="${WFGG_DEV_HUB_RADAR_QUALITY_REV:-}"
PROJECT="wfgg-radar"
RAW_ARCHIVE="https://codeload.github.com/chachasan090375/WfGg/tar.gz/${REV}"
WORK="$(mktemp -d /tmp/chacha-radar-quality.XXXXXX)"
ARCHIVE="$WORK/repo.tar.gz"
RUNTIME="/opt/chacha-dev/runtime"
GRAPH="$RUNTIME/plans/$PROJECT/radar-cluster-quality-diagnostic.task-graph.json"

cleanup(){ rm -rf "$WORK"; }
trap cleanup EXIT

die(){ echo "RADAR_CLUSTER_QUALITY_E2E=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR CLUSTER QUALITY DIAGNOSTIC ==="
echo "SOURCE_REV=$REV"
...
echo "RADAR_CLUSTER_QUALITY_CONTRACT_TESTS=PASS"

WFGG_DEV_HUB_RADAR_ADAPTER_REV="$REV" bash dev-hub/bin/install-radar-runtime-adapter-pilot.sh
echo "RADAR_CLUSTER_QUALITY_ADAPTER_PROVISIONING=PASS"
...
print('RADAR_CLUSTER_QUALITY_REGISTRY=ENABLED')
PY
...
graph={
  ...
  'tasks':[{
    'id':'radar-runtime:cluster-quality-diagnostic',
    'kind':'runtime-diagnostic',
    'description':'Diagnose SERVER_CLUSTER_CATALOG_QUALITY_UNAVAILABLE through the ENABLED ChaCha DEV Radar runtime adapter without mutating Radar or Collector data.',
    'owner_role':'sre-observability',
    'capabilities':['radar-runtime-inspect'],
    'permission':'read',
    'depends_on':[],
    'outputs':[{'type':'artifact','id':'radar-cluster-quality-diagnostic'}],
    'verification':{
      'mode':'machine',
    ...
    'metadata':{'radar_runtime':{'action':'cluster-quality-diagnostic'}}
  }],
  'summary':{
...
print('RADAR_CLUSTER_QUALITY_GRAPH='+graph_path)
print('RADAR_CLUSTER_QUALITY_PROJECT_STAGE='+stage)
PY
...
t=tasks[0]
assert t['task_id']=='radar-runtime:cluster-quality-diagnostic',t
...
assert tasks[0]['task_id']=='radar-runtime:cluster-quality-diagnostic',tasks[0]
...
assert x['task_id']=='radar-runtime:cluster-quality-diagnostic',x
assert x['status']=='OK',x
...
print('RADAR_CLUSTER_QUALITY_DIAGNOSTIC=PASS')
print('DB_PATH_SOURCE='+str(d.get('db_path_source')))
print('DB_EXISTS='+('YES' if d.get('db_exists') else 'NO'))
print('DB_READABLE='+('YES' if d.get('db_readable') else 'NO'))
print('PYTHON3_AVAILABLE='+('YES' if d.get('python3_path') else 'NO'))
print('CYCLES_TABLE_PRESENT='+('YES' if d.get('cycles_table_present') else 'NO'))
print('CYCLES_COLUMNS='+','.join(d.get('cycles_columns') or []))
print('MISSING_COLUMNS='+','.join(d.get('missing_columns') or []))
print('CYCLE_COUNT='+str(d.get('cycle_count')))
print('STATUS_COUNTS='+json.dumps(d.get('status_counts') or {},sort_keys=True,separators=(',',':')))
print('CYCLES_WITH_ERROR='+str(d.get('cycles_with_error')))
print('FEDERATED_CYCLES='+str(d.get('federated_cycles')))
print('QUALITY_GATE_READY='+('YES' if d.get('quality_gate_ready') else 'NO'))
...
echo "RADAR_CLUSTER_QUALITY_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER"
echo "RADAR_CLUSTER_QUALITY_E2E=PASS"
