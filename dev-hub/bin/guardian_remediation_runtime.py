...
def current_directive(*,actor:str,subject_role:str,project_id:str|None,run_id:str|None=None,index_path:Path=DEFAULT_INDEX)->dict[str,Any]|None:
    ...
    for d in items:
        ...
        dr=str(d.get("run_id") or "")
        rules=[str(x) for x in (d.get("rule_codes") or [])]
        if run:
            if dr and dr!=run:continue
            if not dr and "POST_ACTION_MISSING" in rules:continue
        elif dr:
            continue
...
def inject_context(context:dict[str,Any],*,actor:str,subject_role:str,project_id:str|None,run_id:str|None=None,index_path:Path=DEFAULT_INDEX)->dict[str,Any]:
    out=dict(context or {})
    d=current_directive(actor=actor,subject_role=subject_role,project_id=project_id,run_id=run_id,index_path=index_path)
    if d:
