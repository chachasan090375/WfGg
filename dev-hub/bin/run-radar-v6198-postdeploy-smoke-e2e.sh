...
REV="${WFGG_DEV_HUB_RADAR_V6198_POSTDEPLOY_SMOKE_REV:-}"
PROJECT="wfgg-radar"
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v6198-postdeploy-smoke.XXXXXX")"
ARCHIVE="$WORK/repo.tar.gz"
GRAPH="$RUNTIME/plans/$PROJECT/radar-v6198-postdeploy-smoke.task-graph.json"

cleanup(){ rm -rf "$WORK"; }
trap cleanup EXIT
die(){ echo "RADAR_V6198_POSTDEPLOY_SMOKE_E2E=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.8 POST-DEPLOY SMOKE ==="
echo "SOURCE_REV=$REV"
...
echo "RADAR_V6198_POSTDEPLOY_SMOKE_CONTRACT_TESTS=PASS"

WFGG_DEV_HUB_RADAR_ADAPTER_REV="$REV" bash dev-hub/bin/install-radar-runtime-adapter-pilot.sh >/dev/null
echo "RADAR_V6198_POSTDEPLOY_SMOKE_ADAPTER_PROVISIONING=PASS"
...
graph={
 ...
 'tasks':[{
   'id':'radar-runtime:sentinel-release-diagnostic-v6198',
...
print('RADAR_V6198_POSTDEPLOY_SMOKE_GRAPH='+graph_path)
print('RADAR_V6198_POSTDEPLOY_SMOKE_PROJECT_STAGE='+stage)
PY
...
print("RADAR_V6198_CLOUDFLARE_HEALTH=PASS")
PY
  then break; fi
...
echo "RADAR_V6198_CLOUDFLARE_UI=PASS"
echo "RADAR_V6198_PRODUCTION_URL=$RADAR_URL"
echo "RADAR_V6198_LIVE_UI_URL=$RADAR_URL/live-radar.html"
...
echo "RADAR_V6198_POSTDEPLOY_SMOKE=PASS"
echo "RADAR_V6198_POSTDEPLOY_SMOKE_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER"
echo "RADAR_V6198_POSTDEPLOY_SMOKE_E2E=PASS"
