#!/usr/bin/env bash
# V6.19.11 Manual Search Stop guard-only pilot runner; qualification workflow must be green before execution.
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v61911-deploy.XXXXXX")"
...
cleanup(){ rm -rf "$WORK"; }
die(){ echo "RADAR_V61911_CHACHA_DEPLOY=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.11 PILOT DEPLOY ==="
...
obj={
  ...
  "scope":{
    ...
    "purpose":"qualify the V6.19.11 manual-search STOP route and cancellation guards without executing a Last War scan or mutating Collector during the guard probe"
  },
  "actor":"human-user",
...
graph={
  ...
  "tasks":[{
    ...
    "kind":"approval",
    "description":"Record explicit human approval for the single V6.19.11 Manual Search Stop pilot window.",
...
result={
  ...
  "observed_at":now,
  "summary":"Human approved the single V6.19.11 Manual Search Stop pilot deployment window.",
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.11 Manual Search Stop pilot approval has been consumed and is revoked after the single pilot window.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_pilot_window":True,
  "status":"REJECTED"
}
...
res={
 ...
 "summary":"Consumed single-window V6.19.11 Manual Search Stop production approval revoked.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","status":"REJECTED"}}],
