...
REV="${WFGG_DEV_HUB_RADAR_V6199_PROD_RECONCILE_REV:-}"
PROJECT="wfgg-radar"
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v6199-prod-reconcile.XXXXXX")"
...
die(){ echo "RADAR_V6199_PROD_RECONCILE=BLOCKED reason=$1"; exit 2; }
cleanup(){ rm -rf "$WORK"; }
...
echo "=== CHACHA DEV RADAR V6.19.9 PRODUCTION SENTINEL RECONCILE ==="
echo "SOURCE_REV=$REV"
...
# approved V6.19.9 artifacts. This happens before approval ingestion, so an
...
for i in $(seq 1 120); do
  if curl -fsSL "$PROD_SUMS_URL?probe=$i" -o "$WORK/production-SHA256SUMS" 2>/dev/null \
    ...
    echo "RADAR_V6199_PRODUCTION_RELEASE_MANIFEST=READY attempt=$i"
    ready=1
    break
  fi
  echo "RADAR_V6199_PRODUCTION_RELEASE_MANIFEST=WAIT attempt=$i"
  sleep 5
done
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v6199-production-promotion-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6199-production-promotion-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.9 production promotion approval has been consumed or the single promotion window has ended.",
...
graph={
 ...
 "tasks":[{
   "id":"approval:radar-v6199-production-revoke","kind":"approval",
   "description":"Revoke consumed V6.19.9 production approval.","owner_role":"project-owner",
...
res={
 ...
 "task_id":"approval:radar-v6199-production-revoke","status":"OK",
 "producer":"human-approval-recorder","observed_at":now,
 "summary":"Consumed V6.19.9 production approval revoked.",
...
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6199-production-promotion.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6199-production-promotion.task-graph.json"
...
obj={
  ...
  "project":"wfgg-radar",
  "scope":{
    "release":"V6.19.9 Autopilot Targeted History Quality",
    "action":"production promotion, Sentinel reconcile and associated Cloudflare surfaces",
    "connector_sha256":"5058159307ccb99e8e631fe71014608934b8575e69d5599e4388550306baab7f",
  ...
  "statement":"J’approuve la promotion production Radar V6.19.9 Autopilot Targeted History Quality et le déploiement des surfaces Cloudflare associées.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_promotion_window":True
}
...
graph={
 ...
 "tasks":[{
   "id":"approval:radar-v6199-production","kind":"approval",
   "description":"Record explicit human approval for V6.19.9 production promotion.","owner_role":"project-owner",
   "capabilities":[],"permission":"read","depends_on":[],
   "outputs":[{"type":"approval","id":"production-release"}],
   "verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},
   "blocking":True
...
res={
 "schema":"chacha.dev/task-result/v1","project":"wfgg-radar",
 "task_id":"approval:radar-v6199-production","status":"OK",
 "producer":"human-approval-recorder","observed_at":now,
 "summary":"Human approved V6.19.9 production promotion.",
 "evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","scope":"radar-v6199-production"}}],
...
GRAPH="$PLAN_DIR/radar-v6199-production-sentinel-reconcile.task-graph.json"
python3 - "$GRAPH" <<'PY'
import json,sys
from datetime import datetime,timezone
g={
 ...
 "tasks":[{
   "id":"radar-runtime:production-sentinel-reconcile-v6199","kind":"runtime-deploy",
   "description":"Trigger Radar Sentinel reconciliation against the published V6.19.9 production release and verify exact runtime SHA.",
   "owner_role":"sre-observability","capabilities":["radar-pilot-control"],
   "permission":"production-deploy","depends_on":[],
   "outputs":[{"type":"gate","id":"radar-v6199-production-runtime"}],
...
print('RADAR_V6199_PRODUCTION_RUNTIME=PASS')
PY
...
echo "RADAR_V6199_COLLECTOR_MUTATION=DISCOVERY_EVIDENCE_TERMINALIZATION_ONLY"
echo "RADAR_V6199_HISTORY_MUTATION=NO"
echo "RADAR_V6199_CHACHA_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER>SENTINEL"
echo "RADAR_V6199_PROD_RECONCILE=PASS"
