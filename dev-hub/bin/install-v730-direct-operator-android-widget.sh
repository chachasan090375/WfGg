...
REV="${CHACHA_DEV_V730_REV:-}"
SOURCE_ROOT="${CHACHA_DEV_V730_SOURCE_ROOT:-}"
...
WORK="$(mktemp -d /tmp/chacha-v730.XXXXXX)"
PREVIOUS=""
ACTIVATED=0
PURGE_COMMITTED=0
SERVE_CONFIGURED=0
...
stage(){ STAGE="$1"; echo "CHACHA_DEV_V730_STAGE=$STAGE"; }
cleanup(){ rm -rf "$WORK" 2>/dev/null || true; }
assert_current(){
  local actual
  actual="$(readlink -f "$CURRENT" 2>/dev/null || true)"
  [ "$actual" = "$RELEASE" ] || { echo "CHACHA_DEV_V730_CURRENT_DRIFT expected=$RELEASE actual=$actual"; return 42; }
  [ "$(cat "$RELEASE/.revision" 2>/dev/null || true)" = "$REV" ] || return 43
  grep -Fq '"version":"7.3.0"' "$RELEASE/dev-hub/bin/autonomous-project-orchestrator.py" || return 44
...
rollback(){
  rc=$?
  if [ "$rc" -ne 0 ]; then
    echo "CHACHA_DEV_V730_FAILURE_STAGE=$STAGE"
    ...
    if [ "$SERVE_CONFIGURED" -eq 1 ]; then
      ...
      echo "CHACHA_DEV_V730_TAILSCALE_8443_ROLLBACK=PASS"
    fi
    ...
    if [ "$PURGE_COMMITTED" -eq 0 ] && [ "$ACTIVATED" -eq 1 ] && [ -n "$PREVIOUS" ] && [ -d "$PREVIOUS" ]; then
      ...
      if [ "$actual" = "$RELEASE" ] || [ -z "$actual" ]; then
        ...
        echo "CHACHA_DEV_V730_RUNTIME_ROLLBACK=PASS"
      else
        echo "CHACHA_DEV_V730_RUNTIME_ROLLBACK=SKIPPED_EXTERNAL_CURRENT actual=$actual"
      fi
...
[ "$(id -u)" -eq 0 ] || { echo "CHACHA_DEV_V730_INSTALL=BLOCKED reason=root_required"; exit 2; }
mkdir -p "$RUNTIME/control" "$RUNTIME/secrets" "$RUNTIME/direct-operator"
exec 9>"$DEPLOY_LOCK"
flock -n 9 || { echo "CHACHA_DEV_V730_INSTALL=BLOCKED reason=platform_deploy_lock_busy"; exit 73; }
printf '%s' "$REV" | grep -Eq '^[0-9a-f]{40}$' || { echo "CHACHA_DEV_V730_INSTALL=BLOCKED reason=pinned_revision_required"; exit 2; }
[ -L "$CURRENT" ] || { echo "CHACHA_DEV_V730_INSTALL=BLOCKED reason=current_release_symlink_missing"; exit 2; }
...
print("CHACHA_DEV_V730_V720_BASELINE=PASS")
...
grep -Fq 'CHACHA_DEV_V730_TRANSLATOR_FLEET_MEMBERSHIP=EXCLUDED_EDGE_SATELLITE' "$WORK/v730.out"
...
echo "CHACHA_DEV_V730_LOCAL_QUALIFICATION=PASS"
...
req=urllib.request.Request("https://api.github.com/repos/chachasan090375/WfGg/actions/runs?"+q,
 headers={"User-Agent":"ChaCha-DEV-V730-Installer/1.0","Accept":"application/vnd.github+json"})
...
print("CHACHA_DEV_V730_EXACT_SHA_ASSURANCE=PASS")
PY
...
grep -Fq '"version":"7.3.0"' "$RELEASE/dev-hub/bin/autonomous-project-orchestrator.py"
...
echo "CHACHA_DEV_V730_COMPILED_RELEASE=PASS"
...
os.chmod(p,0o600)
print("CHACHA_DEV_V730_OPERATOR_IDENTITY=PASS")
PY
  AUTH_CREATED=1
else
  chmod 600 "$AUTH_FILE"
...
print("CHACHA_DEV_V730_OPERATOR_IDENTITY=EXISTING")
PY
fi
...
print("CHACHA_DEV_V730_DIRECT_OPERATOR_HEALTH=PASS")
PY
...
echo "CHACHA_DEV_V730_PRIVATE_TAILSCALE_SERVE=PASS"
echo "CHACHA_DEV_V730_EXISTING_FUNNEL_PRESERVED=YES"
...
echo "CHACHA_DEV_V730_GUARDIAN_COVERAGE=PASS"
...
print("CHACHA_DEV_V730_REAL_DIRECT_STATUS=PASS")
PY
...
if not translation:
    ...
    pass
print("CHACHA_DEV_V730_REAL_FUNCTIONAL_TRANSLATION=PASS")
...
print("CHACHA_DEV_V730_RETENTION=PASS")
PY
PURGE_COMMITTED=1
...
assert_current
mkdir -p "$EVIDENCE"
python3 - "$EVIDENCE/v730-direct-operator-android-widget-$STAMP.json" "$REV" "$STAMP" "$WORK/status-job.json" "$WORK/instruction-job.json" <<'PY'
import json,pathlib,subprocess,sys
status=json.load(open(sys.argv[4]));instruction=json.load(open(sys.argv[5]))
rels=[p for p in pathlib.Path("/opt/chacha-dev/platform/releases").iterdir() if p.is_dir()]
out={
 "schema":"chacha.dev/v730-direct-operator-android-widget-evidence/v1",
 "revision":sys.argv[2],"observed_at":sys.argv[3],"platform_version":"7.3.0",
 ...
 "android_widget":{"build_workflow":"ChaCha DEV V7.3 Android Operator Widget build",
   "exact_revision_success":True,"embedded_secret":False},
...
echo "CHACHA_DEV_V730_INSTALL=PASS"
echo "CHACHA_DEV_V730_DIRECT_OPERATOR=PASS"
echo "CHACHA_DEV_V730_PRIVATE_TAILSCALE_SERVE=PASS"
echo "CHACHA_DEV_V730_REAL_DIRECT_STATUS=PASS"
echo "CHACHA_DEV_V730_REAL_FUNCTIONAL_TRANSLATION=PASS"
echo "CHACHA_DEV_V730_FUNCTIONAL_TRANSLATOR_SCOPE=PLATFORM_EDGE_SATELLITE"
