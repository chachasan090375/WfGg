...
python3 -m py_compile   "$SRC/dev-hub/bin/controlled-production-adapter.py"   "$SRC/dev-hub/bin/controlled-production-handoff-controller.py"   "$SRC/dev-hub/tests/test_v639_controlled_production_handoff.py"
for f in   "$SRC/dev-hub/config/controlled-production-handoff.v1.json"   "$SRC/dev-hub/config/controlled-production-adapter-dry-run.v1.json"   "$SRC/dev-hub/config/lifecycle.v1.json"   "$SRC/dev-hub/config/project-control.v1.json"; do
...
for marker in   CHACHA_DEV_V639_PRODUCTION_APPROVAL_BOUNDARY=PASS   CHACHA_DEV_V639_RELEASE_OPERATE_TASK_GRAPH=PASS   CHACHA_DEV_V639_TRUE_TWO_PHASE_DEPLOYMENT_HANDOFF=PASS   CHACHA_DEV_V639_PROTECTED_APPROVAL_TRANSACTION=PASS   CHACHA_DEV_V639_CONTROLLED_ADAPTER_DRY_RUN=PASS   CHACHA_DEV_V639_ROLLBACK_PROVEN=PASS   CHACHA_DEV_V639_OPERATE_ADVANCED=NO   CHACHA_DEV_V639_REAL_PRODUCTION_TARGET=NO   CHACHA_DEV_V639_AUTOMATIC_EXTERNAL_SPEND_EUR=0; do
