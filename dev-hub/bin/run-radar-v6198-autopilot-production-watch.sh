...
REV="${WFGG_DEV_HUB_RADAR_V6198_AUTOPILOT_WATCH_REV:-}"
...
die(){ echo "RADAR_V6198_AUTOPILOT_WATCH=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.8 AUTOPILOT PRODUCTION WATCH ==="
echo "SOURCE_REV=$REV"
...
echo "RADAR_V6198_AUTOPILOT_WATCH_PREFLIGHT=PASS"
...
while time.time()<deadline:
    ...
    if not target_row:
        print("RADAR_V6198_AUTOPILOT_WATCH_FAIL=TARGET_CYCLE_NOT_FOUND",flush=True)
        raise SystemExit(3)
    ...
    if state != last:
        print("RADAR_V6198_AUTOPILOT_CYCLE_51_STATUS="+str(target_row['status']),flush=True)
        print("RADAR_V6198_AUTOPILOT_CYCLE_51_ERROR="+(str(target_row['error']) or 'NONE'),flush=True)
        print("RADAR_V6198_AUTOPILOT_CYCLE_51_FINISHED_AT="+(str(target_row['finished_at']) or 'NONE'),flush=True)
        if next_row:
            print("RADAR_V6198_AUTOPILOT_NEXT_CYCLE_ID="+str(next_row['id']),flush=True)
            print("RADAR_V6198_AUTOPILOT_NEXT_QUERY="+str(next_row['query']),flush=True)
            print("RADAR_V6198_AUTOPILOT_NEXT_STATUS="+str(next_row['status']),flush=True)
        last=state
    ...
    if status in ('SUCCESS','FAILED') and finished:
        ...
        print("RADAR_V6198_AUTOPILOT_CYCLE_51_TERMINAL=PASS",flush=True)
        if status=='FAILED':
            err=str(target_row['error'] or '')
            if not err:
                print("RADAR_V6198_AUTOPILOT_WATCH_FAIL=FAILED_WITHOUT_ERROR",flush=True)
                raise SystemExit(4)
            if err.endswith('_CYCLE_TERMINALIZATION_FAILED'):
                print("RADAR_V6198_AUTOPILOT_WATCH_FAIL=TERMINALIZATION_RETRIES_EXHAUSTED",flush=True)
                raise SystemExit(5)

    if next_row:
        next_seen=True
        print("RADAR_V6198_AUTOPILOT_CONTINUATION_OBSERVED=PASS",flush=True)

    if target_terminal and next_seen:
        print("RADAR_V6198_AUTOPILOT_PRODUCTION_WATCH=PASS",flush=True)
        raise SystemExit(0)
...
print("RADAR_V6198_AUTOPILOT_WATCH_FAIL=TIMEOUT",flush=True)
raise SystemExit(6)
PY
...
echo "RADAR_V6198_AUTOPILOT_WATCH=PASS"
