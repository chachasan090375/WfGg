...
class ProgressStore:
    ...
    def blank(self)->dict[str,Any]:
        ...
        return {"schema":SCHEMA,"status":"IDLE","overall_percent":0,"headline":"ChaCha est prêt ✨",
    ...
    def begin(self,operation_id:str,title:str,project_id:str)->dict[str,Any]:
        x=self.blank();x.update({"status":"RUNNING","overall_percent":3,
    ...
    def update(self,module_id:str,percent:int,state:str,detail:str,overall_percent:int|None=None,
        ...
        if overall_percent is not None:x["overall_percent"]=clamp(overall_percent)
    ...
    def complete(self,headline:str,status:str="COMPLETE")->dict[str,Any]:
        x=self.snapshot();x["overall_percent"]=100;x["status"]=status;x["headline"]=headline
...
def main()->int:
    ...
    else:out=store.fail(a.headline)
