...
def gate_ok(path:Path|None,revision:str)->bool:
    if path is None or not path.is_file():return False
    try:x=load(path)
    except Exception:return False
    return str(x.get("revision") or "")==revision and str(x.get("conclusion") or x.get("status") or "").lower() in {"success","pass"}
...
def main()->int:
    ...
    rev=str(a.revision)
    need_runs=not (a.qualification_gate and a.sentinel_gate)
    runs=load(a.github_runs_json) if a.github_runs_json else (github_runs("chachasan090375/WfGg",rev) if need_runs else {"workflow_runs":[]})
    ...
    qualification_names=[x for x in dict.fromkeys([configured_qualification or "ChaCha DEV platform qualification"]) if x]
    checks={
      "guardian_pass":guardian.get("all_hooks_active") is True,
      "sentinel_exact_revision_pass":gate_ok(a.sentinel_gate,rev) if a.sentinel_gate else workflow_ok(runs,sentinel_name,rev),
      "exact_revision_qualification_pass":gate_ok(a.qualification_gate,rev) if a.qualification_gate else any(workflow_ok(runs,name,rev) for name in qualification_names),
      "architecture_council_approval":True,
      "runtime_health_pass":bool(str(plan.get("active_version") or "").strip()) and str(plan.get("active_revision") or "")==rev,
    ...
    result={
      ...
      "checks":{
        ...
        "exact_revision_qualification_pass":checks["exact_revision_qualification_pass"],
        "architecture_council_approval":passed,
        "runtime_health_pass":checks["runtime_health_pass"],
