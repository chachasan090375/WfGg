...
REV="${CHACHA_DEV_V815_REV:-}"
SOURCE_ROOT="${CHACHA_DEV_V815_SOURCE_ROOT:-/opt/chacha-dev/platform/current}"
UNIT_DIR="/etc/systemd/system"
WORK="$(mktemp -d /tmp/chacha-v815.XXXXXX)"
STAGE="bootstrap"
TOUCHED=0
...
stage(){ STAGE="$1"; echo "CHACHA_DEV_V815_STAGE=$STAGE"; }
cleanup(){ rm -rf "$WORK" 2>/dev/null || true; }
rollback(){
  rc=$?
  if [ "$rc" -ne 0 ]; then
    echo "CHACHA_DEV_V815_FAILURE_STAGE=$STAGE"
    if [ "$TOUCHED" -eq 1 ]; then
      ...
      echo "CHACHA_DEV_V815_UNIT_ROLLBACK=PASS"
    fi
  fi
  cleanup
  exit "$rc"
...
[ "$(id -u)" -eq 0 ] || { echo "CHACHA_DEV_V815_INSTALL=BLOCKED reason=root_required"; exit 2; }
SRC="$(readlink -f "$SOURCE_ROOT")"
[ -d "$SRC/dev-hub" ] || { echo "CHACHA_DEV_V815_INSTALL=BLOCKED reason=source_root_invalid"; exit 2; }
if [ -n "$REV" ]; then
  printf '%s' "$REV" | grep -Eq '^[0-9a-f]{40}$' || { echo "CHACHA_DEV_V815_INSTALL=BLOCKED reason=pinned_revision_invalid"; exit 2; }
  ACTUAL="$(cat "$SRC/.revision" 2>/dev/null || true)"
  [ "$ACTUAL" = "$REV" ] || { echo "CHACHA_DEV_V815_INSTALL=BLOCKED reason=revision_mismatch actual=$ACTUAL"; exit 2; }
fi
for cmd in systemctl systemd-analyze install python3 readlink; do
  command -v "$cmd" >/dev/null || { echo "CHACHA_DEV_V815_INSTALL=BLOCKED reason=missing_command:$cmd"; exit 2; }
done
for unit in "${UNITS[@]}"; do
  [ -f "$SRC/dev-hub/systemd/$unit" ] || { echo "CHACHA_DEV_V815_INSTALL=BLOCKED reason=missing_unit:$unit"; exit 2; }
done
...
echo "CHACHA_DEV_V815_SYSTEMD_VERIFY=PASS"
...
for timer in "${TIMERS[@]}"; do
  ...
  [ -n "$NEXT" ] && [ "$NEXT" != "infinity" ] || { echo "CHACHA_DEV_V815_INSTALL=BLOCKED reason=timer_not_scheduled:$timer"; exit 3; }
  echo "CHACHA_DEV_V815_TIMER_NEXT=$timer:$NEXT"
done
...
echo "CHACHA_DEV_V815_RETRY_QUEUES=PASS"
echo "CHACHA_DEV_V815_AUTOMATIC_EXTERNAL_SPEND_EUR=0"
echo "CHACHA_DEV_V815_INSTALL=PASS"
