...
PROJECT="wfgg-radar"
PILOT_REV="c0ceb376b8735d3aa037608c91b8c0702f27d9f5"
PILOT_BRANCH="radar-v6193-seed-scout"
CONNECTOR_SHA="73b64743031e4a47212dfdafae8ad67551f88d131831e70cb08a40957824f9a8"
NATIVE_SHA="274d040f5294cb09422e5d55cc4b5335ac7739924c33dcb67b3f279645814900"
INSTALLER="radar-vps/install-v6193-pilot.sh"
PROBE="radar-vps/probe-v6193-pilot-runtime.sh"
RAW_ARCHIVE="https://codeload.github.com/chachasan090375/WfGg/tar.gz/${REV}"
WORK="$(mktemp -d /tmp/chacha-radar-v6193-deploy.XXXXXX)"
...
cleanup(){ rm -rf "$WORK"; }
die(){ echo "RADAR_V6193_CHACHA_DEPLOY=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.3 PILOT DEPLOY ==="
...
# ---------------------------------------------------------------------------
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6193-pilot.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6193-pilot.task-graph.json"
...
obj={
  ...
  "scope":{
    ...
    "change":"read-only one-region Seed Scout",
    "purpose":"qualify the V6.19.3 Seed Scout routes and SHA-pinned runtime without executing a game scan"
  },
  "actor":"human-user",
...
graph={
  ...
  "generated_at":now,
  "tasks":[{
    "id":"approval:radar-v6193-pilot",
    "kind":"approval",
    "description":"Record explicit human approval for the single V6.19.3 Seed Scout Radar pilot window.",
...
result={
  "schema":"chacha.dev/task-result/v1",
  "project":"wfgg-radar",
  "task_id":"approval:radar-v6193-pilot",
  "status":"OK",
  "producer":"human-approval-recorder",
  "observed_at":now,
  "summary":"Human approved the single V6.19.3 Seed Scout Radar pilot deployment window.",
  "evidence":[{"kind":"human-approval","source":src,"digest":digest,"details":{"approval_id":"production-release","scope":"radar-v6193-pilot"}}],
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v6193-pilot-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6193-pilot-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.3 Seed Scout pilot approval has been consumed and is revoked after the single pilot window.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_pilot_window":True,
  "status":"REJECTED"
}
...
graph={
 ...
 "tasks":[{"id":"approval:radar-v6193-pilot-revoke","kind":"approval","description":"Revoke the consumed one-shot production approval.","owner_role":"project-owner","capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"production-release"}],"verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],
 "summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={
 "schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6193-pilot-revoke","status":"OK","producer":"human-approval-recorder","observed_at":now,
 "summary":"Consumed single-window V6.19.3 Seed Scout production approval revoked.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","status":"REJECTED"}}],
...
close_pilot () {
  ...
  CLOSE_ATTEMPTED=1
  local graph="$PLAN_DIR/radar-v6193-close.task-graph.json"
  python3 - "$graph" <<'PY'
import json,sys
from datetime import datetime,timezone
...
PY
  if rr="$(run_graph "$graph" RADAR_V6193_CLOSE | tail -1)"; then
    python3 - "$rr" <<'PY'
import json,sys
x=json.load(open(sys.argv[1],encoding='utf-8'))
tasks=[t for w in x.get('waves') or [] for t in w.get('tasks') or []]
ok=len(tasks)==1 and tasks[0].get('status')=='SUCCEEDED'
print('RADAR_V6193_PILOT_CLOSE='+('PASS' if ok else 'FAIL'))
raise SystemExit(0 if ok else 1)
PY
  else
    echo "RADAR_V6193_PILOT_CLOSE=FAIL"
    return 1
  fi
}
...
# ---------------------------------------------------------------------------
# 3) ChaCha DEV governed pilot: open -> install -> read-only Seed Scout route probe.
#    No game scan is executed by the probe; close is guaranteed in the EXIT path.
# ---------------------------------------------------------------------------
PILOT_GRAPH="$PLAN_DIR/radar-v6193-pilot.task-graph.json"
...
g={
 ...
 "tasks":[
  {"id":"radar-runtime:pilot-open-v6193","kind":"runtime-deploy","description":"Open the protected Radar V6.19.3 Seed Scout pilot window.","owner_role":"sre-observability","capabilities":["radar-pilot-control"],"permission":"production-deploy","depends_on":[],"outputs":[{"type":"gate","id":"radar-pilot-window"}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-open"}}},
  {"id":"radar-runtime:pilot-install-v6193","kind":"runtime-deploy","description":"Install SHA-pinned V6.19.3 Seed Scout pilot binaries with installer rollback protection.","owner_role":"release","capabilities":["radar-pilot-control"],"permission":"production-deploy","depends_on":["radar-runtime:pilot-open-v6193"],"outputs":[{"type":"artifact","id":rev}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-install","revision":rev,"installer":installer,"expected_connector_sha256":csha,"expected_native_sha256":nsha}}},
  {"id":"radar-runtime:pilot-probe-v6193","kind":"runtime-diagnostic","description":"Verify the installed V6.19.3 Seed Scout routes and guards without executing a game scan.","owner_role":"sre-observability","capabilities":["radar-runtime-inspect"],"permission":"read","depends_on":["radar-runtime:pilot-install-v6193"],"outputs":[{"type":"artifact","id":"radar-v6193-runtime-probe"}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-probe","revision":rev,"installer":installer,"probe":probe,"expected_connector_sha256":csha,"expected_native_sha256":nsha}}}
 ],
...
PILOT_RUN="$(run_graph "$PILOT_GRAPH" RADAR_V6193_PILOT | tail -1)"
PILOT_OPENED=1
...
for tid in ("radar-runtime:pilot-open-v6193","radar-runtime:pilot-install-v6193","radar-runtime:pilot-probe-v6193"):
    t=by.get(tid) or {}
...
probe=by.get("radar-runtime:pilot-probe-v6193") or {}
...
print("RADAR_V6193_RUNTIME_ROUTE_GUARDS=PASS")
print("RADAR_V6193_GAME_SCAN_EXECUTED=NO")
print("RADAR_V6193_COLLECTOR_MUTATION=NO")
PY
...
echo "RADAR_V6193_PRODUCTION_APPROVAL_STANDING=NO"
echo "RADAR_V6193_PILOT_WINDOW_CLOSED=YES"
echo "RADAR_V6193_CHACHA_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER"
echo "RADAR_V6193_CHACHA_DEPLOY=PASS"
