...
REV="${WFGG_DEV_HUB_RADAR_V6196_PROD_RECONCILE_REV:-}"
PROJECT="wfgg-radar"
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v6196-prod-reconcile.XXXXXX")"
...
die(){ echo "RADAR_V6196_PROD_RECONCILE=BLOCKED reason=$1"; exit 2; }
cleanup(){ rm -rf "$WORK"; }
...
echo "=== CHACHA DEV RADAR V6.19.6 PRODUCTION SENTINEL RECONCILE ==="
echo "SOURCE_REV=$REV"
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v6196-production-promotion-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6196-production-promotion-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.6 production promotion approval has been consumed or the single promotion window has ended.",
...
graph={
 ...
 "tasks":[{
   "id":"approval:radar-v6196-production-revoke","kind":"approval",
   "description":"Revoke consumed V6.19.6 production approval.","owner_role":"project-owner",
...
res={
 ...
 "task_id":"approval:radar-v6196-production-revoke","status":"OK",
 "producer":"human-approval-recorder","observed_at":now,
 "summary":"Consumed V6.19.6 production approval revoked.",
...
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6196-production-promotion.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6196-production-promotion.task-graph.json"
...
obj={
  ...
  "project":"wfgg-radar",
  "scope":{
    "release":"V6.19.6 Autopilot Continue-on-No-Data",
    "action":"production promotion, Sentinel reconcile and associated Cloudflare surfaces",
    "connector_sha256":"b7ff60476f3afe7c8bcb9b78963c46daa4e8b5507490cc83cbcbc4ced63e7c35",
  ...
  "statement":"J’approuve la promotion production Radar V6.19.6 Autopilot Continue-on-No-Data et le déploiement des surfaces Cloudflare associées.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_promotion_window":True
}
...
graph={
 ...
 "tasks":[{
   "id":"approval:radar-v6196-production","kind":"approval",
   "description":"Record explicit human approval for V6.19.6 production promotion.","owner_role":"project-owner",
   "capabilities":[],"permission":"read","depends_on":[],
   "outputs":[{"type":"approval","id":"production-release"}],
   "verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},
   "blocking":True
...
res={
 "schema":"chacha.dev/task-result/v1","project":"wfgg-radar",
 "task_id":"approval:radar-v6196-production","status":"OK",
 "producer":"human-approval-recorder","observed_at":now,
 "summary":"Human approved V6.19.6 production promotion.",
 "evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","scope":"radar-v6196-production"}}],
...
GRAPH="$PLAN_DIR/radar-v6196-production-sentinel-reconcile.task-graph.json"
python3 - "$GRAPH" <<'PY'
import json,sys
from datetime import datetime,timezone
g={
 ...
 "tasks":[{
   "id":"radar-runtime:production-sentinel-reconcile-v6196","kind":"runtime-deploy",
   "description":"Trigger Radar Sentinel reconciliation against the published V6.19.6 production release and verify exact runtime SHA.",
   "owner_role":"sre-observability","capabilities":["radar-pilot-control"],
   "permission":"production-deploy","depends_on":[],
   "outputs":[{"type":"gate","id":"radar-v6196-production-runtime"}],
...
print('RADAR_V6196_PRODUCTION_RUNTIME=PASS')
PY
...
echo "RADAR_V6196_LASTWAR_MUTATION=NO"
echo "RADAR_V6196_CHACHA_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER>SENTINEL"
echo "RADAR_V6196_PROD_RECONCILE=PASS"
