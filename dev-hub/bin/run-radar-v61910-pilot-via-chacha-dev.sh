#!/usr/bin/env bash
# V6.19.10 Autopilot Continue-on-Partial-Limit guard-only pilot runner; qualification workflow must be green before execution.
# Radar runtime adapter provisioning revision: 1.5.4 (immutable version bump after digest change).
set -Eeuo pipefail

REV="${WFGG_DEV_HUB_RADAR_V61910_DEPLOY_REV:-}"
PROJECT="wfgg-radar"
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v61910-deploy.XXXXXX")"
...
cleanup(){ rm -rf "$WORK"; }
die(){ echo "RADAR_V61910_CHACHA_DEPLOY=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.10 PILOT DEPLOY ==="
...
# ---------------------------------------------------------------------------
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v61910-pilot.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v61910-pilot.task-graph.json"
...
obj={
  ...
  "scope":{
    ...
    "change":"Autopilot continue-on-partial-limit",
    "purpose":"qualify V6.19.10 seed-local partial-limit continuation and safety guards without executing a Last War scan or mutating Collector during the guard probe"
  },
  "actor":"human-user",
  "source":"chat-explicit-approval",
  "statement":"J’approuve le PILOT Radar V6.19.10 Autopilot Continue-on-Partial-Limit révision 6761a1b7383143a40883af3373d6fd9d43a80359 avec probe de garde sans scan de jeu.",
...
graph={
  ...
  "tasks":[{
    ...
    "kind":"approval",
    "description":"Record explicit human approval for the single V6.19.10 Continue-on-Partial-Limit pilot window.",
...
result={
  ...
  "observed_at":now,
  "summary":"Human approved the single V6.19.10 Continue-on-Partial-Limit pilot deployment window.",
  "evidence":[{"kind":"human-approval","source":src,"digest":digest,"details":{"approval_id":"production-release","scope":"radar-v61910-pilot"}}],
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v61910-pilot-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v61910-pilot-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.10 Continue-on-Partial-Limit pilot approval has been consumed and is revoked after the single pilot window.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_pilot_window":True,
  "status":"REJECTED"
}
...
graph={
 ...
 "tasks":[{"id":"approval:radar-v61910-pilot-revoke","kind":"approval","description":"Revoke the consumed one-shot production approval.","owner_role":"project-owner","capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"production-release"}],"verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],
 "summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={
 "schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v61910-pilot-revoke","status":"OK","producer":"human-approval-recorder","observed_at":now,
 "summary":"Consumed single-window V6.19.10 Continue-on-Partial-Limit production approval revoked.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","status":"REJECTED"}}],
...
echo "RADAR_V61910_CHACHA_DEPLOY=PASS"
