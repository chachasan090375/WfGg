...
WORK="$(mktemp -d /tmp/chacha-radar-census-frontier.XXXXXX)"
ARCHIVE="$WORK/repo.tar.gz"
RUNTIME="/opt/chacha-dev/runtime"
GRAPH="$RUNTIME/plans/$PROJECT/radar-census-frontier-diagnostic.task-graph.json"

cleanup(){ rm -rf "$WORK"; }
trap cleanup EXIT
die(){ echo "RADAR_CENSUS_FRONTIER_E2E=BLOCKED reason=$1"; exit 2; }
...
echo "RADAR_CENSUS_FRONTIER_CONTRACT_TESTS=PASS"

WFGG_DEV_HUB_RADAR_ADAPTER_REV="$REV" bash dev-hub/bin/install-radar-runtime-adapter-pilot.sh
echo "RADAR_CENSUS_FRONTIER_ADAPTER_PROVISIONING=PASS"
...
graph={
  ...
  'tasks':[{
    'id':'radar-runtime:census-frontier-diagnostic',
    'kind':'runtime-diagnostic',
    'description':'Inspect aggregate server census and population-floor candidates after V6.19.2 catalog returned no frontier seed.',
    'owner_role':'sre-observability',
    'capabilities':['radar-runtime-inspect'],
    'permission':'read',
    'depends_on':[],
    'outputs':[{'type':'artifact','id':'radar-census-frontier-diagnostic'}],
    'verification':{
      'mode':'machine',
    ...
    'metadata':{'radar_runtime':{'action':'census-frontier-diagnostic'}}
  }],
  'summary':{
...
print('RADAR_CENSUS_FRONTIER_GRAPH='+graph_path)
print('RADAR_CENSUS_FRONTIER_PROJECT_STAGE='+stage)
PY
...
assert tasks[0]['task_id']=='radar-runtime:census-frontier-diagnostic',tasks[0]
...
assert x['task_id']=='radar-runtime:census-frontier-diagnostic',x
assert x['status']=='OK',x
assert x['producer']=='radar-runtime-adapter',x
ev=x.get('evidence') or []
assert ev,x
d=ev[0].get('details') or {}
print('RADAR_CENSUS_FRONTIER_DIAGNOSTIC=PASS')
print('HTTP_STATUS='+str(d.get('http_status')))
print('ELAPSED_MS='+str(d.get('elapsed_ms')))
print('CENSUS_VERSION='+str(d.get('census_version')))
print('SERVER_COUNT='+str(d.get('server_count')))
print('SEED_FLOOR='+str(d.get('seed_floor')))
print('ELIGIBLE_COUNT='+str(d.get('eligible_count')))
print('NOISE_COUNT='+str(d.get('noise_count')))
print('ELIGIBLE_SERVERS='+json.dumps(d.get('eligible_servers') or [],sort_keys=True,separators=(',',':')))
print('NOISE_SERVERS='+json.dumps(d.get('noise_servers') or [],sort_keys=True,separators=(',',':')))
...
echo "RADAR_CENSUS_FRONTIER_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER"
echo "RADAR_CENSUS_FRONTIER_E2E=PASS"
