#!/usr/bin/env bash
# V6.19.7 Stale Cycle Recovery Autopilot guard-only pilot runner; qualification workflow must be green before execution.
# Radar runtime adapter provisioning revision: 1.5.4 (immutable version bump after digest change).
set -Eeuo pipefail

REV="${WFGG_DEV_HUB_RADAR_V6197_DEPLOY_REV:-}"
PROJECT="wfgg-radar"
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v6197-deploy.XXXXXX")"
...
cleanup(){ rm -rf "$WORK"; }
die(){ echo "RADAR_V6197_CHACHA_DEPLOY=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.7 PILOT DEPLOY ==="
...
# ---------------------------------------------------------------------------
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6197-pilot.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6197-pilot.task-graph.json"
...
obj={
  ...
  "scope":{
    ...
    "change":"Autopilot stale-cycle recovery orchestration",
    "purpose":"qualify the V6.19.7 Autopilot routes, SHA-pinned runtime and safety guards without executing a Last War scan or mutating Collector during the guard probe"
  },
  "actor":"human-user",
  "source":"chat-explicit-approval",
  "statement":"J’approuve le PILOT Radar V6.19.7 Stale Cycle Recovery révision 633364c2f2f60aaef6967126df7d25a167f972f3 avec probe de garde sans scan de jeu.",
...
graph={
  ...
  "generated_at":now,
  "tasks":[{
    "id":"approval:radar-v6197-pilot",
    "kind":"approval",
    "description":"Record explicit human approval for the single V6.19.7 Autopilot Radar pilot window.",
...
result={
  "schema":"chacha.dev/task-result/v1",
  "project":"wfgg-radar",
  "task_id":"approval:radar-v6197-pilot",
  "status":"OK",
  "producer":"human-approval-recorder",
  "observed_at":now,
  "summary":"Human approved the single V6.19.7 Autopilot Radar pilot deployment window.",
  "evidence":[{"kind":"human-approval","source":src,"digest":digest,"details":{"approval_id":"production-release","scope":"radar-v6197-pilot"}}],
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v6197-pilot-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6197-pilot-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.7 Autopilot pilot approval has been consumed and is revoked after the single pilot window.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_pilot_window":True,
  "status":"REJECTED"
}
...
graph={
 ...
 "tasks":[{"id":"approval:radar-v6197-pilot-revoke","kind":"approval","description":"Revoke the consumed one-shot production approval.","owner_role":"project-owner","capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"production-release"}],"verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],
 "summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={
 "schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6197-pilot-revoke","status":"OK","producer":"human-approval-recorder","observed_at":now,
 "summary":"Consumed single-window V6.19.7 Autopilot production approval revoked.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","status":"REJECTED"}}],
