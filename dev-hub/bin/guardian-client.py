...
def register_contract(policy_path:Path,contract:Path)->int:
    _,url,key=policy_values(policy_path)
    if not key.is_file():return 30
    payload=load(contract)
    body=json.dumps(payload,sort_keys=True,ensure_ascii=False,separators=(",",":")).encode()
    try:
        status,x=http_json(signed_request("POST",url+"/v1/dynamic-contracts/register",key,body))
    except Exception as exc:
        print(json.dumps({"schema":"chacha.dev/guardian-client-result/v1","status":"UNAVAILABLE","reason":str(exc)[:300]}))
        return 30
    print(json.dumps(x,ensure_ascii=False))
...
def register_component_contract(policy_path:Path,contract:Path)->int:
    _,url,key=policy_values(policy_path)
    if not key.is_file():return 30
    payload=load(contract)
    body=json.dumps(payload,sort_keys=True,ensure_ascii=False,separators=(",",":")).encode()
    try:
        status,x=http_json(signed_request("POST",url+"/v1/dynamic-components/register",key,body))
    except Exception as exc:
        print(json.dumps({"schema":"chacha.dev/guardian-client-result/v1","status":"UNAVAILABLE","reason":str(exc)[:300]}))
        return 30
    print(json.dumps(x,ensure_ascii=False))
...
def functional_acceptance(policy_path:Path,project_id:str,revision:str,contract:Path,acceptance:Path)->int:
    _,url,key=policy_values(policy_path)
    if not key.is_file():return 30
    payload={"schema":"chacha.dev/guardian-functional-acceptance-request/v1",
             "project_id":project_id,"revision":revision,
             "contract":load(contract),"acceptance":load(acceptance)}
    body=json.dumps(payload,sort_keys=True,ensure_ascii=False,separators=(",",":")).encode()
    try:
        status,x=http_json(signed_request("POST",url+"/v1/functional-acceptance",key,body))
    except Exception as exc:
        print(json.dumps({"schema":"chacha.dev/guardian-client-result/v1","status":"UNAVAILABLE","reason":str(exc)[:300]}))
        return 30
    print(json.dumps(x,ensure_ascii=False))
