#!/usr/bin/env bash
# V6.19.4 Autopilot guard-only pilot runner; qualification workflow must be green before execution.
...
PROJECT="wfgg-radar"
PILOT_REV="a426ad7c5d5f7c9e73908f3b54e4b2bac24a77e9"
PILOT_BRANCH="radar-v6194-autopilot"
CONNECTOR_SHA="01ba325e8b02c16d22c197960172f6a9a252ac2f0d43ddefef893f6783846e52"
...
WORK="$(mktemp -d /tmp/chacha-radar-v6194-deploy.XXXXXX)"
...
cleanup(){ rm -rf "$WORK"; }
die(){ echo "RADAR_V6194_CHACHA_DEPLOY=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.4 PILOT DEPLOY ==="
...
# ---------------------------------------------------------------------------
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6194-pilot.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6194-pilot.task-graph.json"
...
obj={
  ...
  "scope":{
    "pilot":"V6.19.4",
    "revision":"a426ad7c5d5f7c9e73908f3b54e4b2bac24a77e9",
...
graph={
  ...
  "generated_at":now,
  "tasks":[{
    "id":"approval:radar-v6194-pilot",
    "kind":"approval",
    "description":"Record explicit human approval for the single V6.19.4 Autopilot Radar pilot window.",
...
result={
  "schema":"chacha.dev/task-result/v1",
  "project":"wfgg-radar",
  "task_id":"approval:radar-v6194-pilot",
  "status":"OK",
  "producer":"human-approval-recorder",
  "observed_at":now,
  "summary":"Human approved the single V6.19.4 Autopilot Radar pilot deployment window.",
  "evidence":[{"kind":"human-approval","source":src,"digest":digest,"details":{"approval_id":"production-release","scope":"radar-v6194-pilot"}}],
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v6194-pilot-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6194-pilot-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.4 Autopilot pilot approval has been consumed and is revoked after the single pilot window.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_pilot_window":True,
  "status":"REJECTED"
}
...
graph={
 ...
 "tasks":[{"id":"approval:radar-v6194-pilot-revoke","kind":"approval","description":"Revoke the consumed one-shot production approval.","owner_role":"project-owner","capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"production-release"}],"verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],
 "summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={
 "schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6194-pilot-revoke","status":"OK","producer":"human-approval-recorder","observed_at":now,
 "summary":"Consumed single-window V6.19.4 Autopilot production approval revoked.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","status":"REJECTED"}}],
...
close_pilot () {
  ...
  CLOSE_ATTEMPTED=1
  local graph="$PLAN_DIR/radar-v6194-close.task-graph.json"
  python3 - "$graph" <<'PY'
import json,sys
from datetime import datetime,timezone
...
PY
  if rr="$(run_graph "$graph" RADAR_V6194_CLOSE | tail -1)"; then
    python3 - "$rr" <<'PY'
import json,sys
x=json.load(open(sys.argv[1],encoding='utf-8'))
tasks=[t for w in x.get('waves') or [] for t in w.get('tasks') or []]
ok=len(tasks)==1 and tasks[0].get('status')=='SUCCEEDED'
print('RADAR_V6194_PILOT_CLOSE='+('PASS' if ok else 'FAIL'))
raise SystemExit(0 if ok else 1)
PY
  else
    echo "RADAR_V6194_PILOT_CLOSE=FAIL"
    return 1
  fi
}
...
# ---------------------------------------------------------------------------
PILOT_GRAPH="$PLAN_DIR/radar-v6194-pilot.task-graph.json"
...
g={
 ...
 "tasks":[
  {"id":"radar-runtime:pilot-open-v6194","kind":"runtime-deploy","description":"Open the protected Radar V6.19.4 Autopilot pilot window.","owner_role":"sre-observability","capabilities":["radar-pilot-control"],"permission":"production-deploy","depends_on":[],"outputs":[{"type":"gate","id":"radar-pilot-window"}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-open"}}},
  {"id":"radar-runtime:pilot-install-v6194","kind":"runtime-deploy","description":"Install SHA-pinned V6.19.4 Autopilot pilot binaries with installer rollback protection.","owner_role":"release","capabilities":["radar-pilot-control"],"permission":"production-deploy","depends_on":["radar-runtime:pilot-open-v6194"],"outputs":[{"type":"artifact","id":rev}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-install","revision":rev,"installer":installer,"expected_connector_sha256":csha,"expected_native_sha256":nsha}}},
  {"id":"radar-runtime:pilot-probe-v6194","kind":"runtime-diagnostic","description":"Verify the installed V6.19.4 Autopilot routes and guards without executing a Last War scan or Collector mutation.","owner_role":"sre-observability","capabilities":["radar-runtime-inspect"],"permission":"read","depends_on":["radar-runtime:pilot-install-v6194"],"outputs":[{"type":"artifact","id":"radar-v6194-runtime-probe"}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-probe","revision":rev,"installer":installer,"probe":probe,"expected_connector_sha256":csha,"expected_native_sha256":nsha}}}
 ],
...
PILOT_RUN="$(run_graph "$PILOT_GRAPH" RADAR_V6194_PILOT | tail -1)"
PILOT_OPENED=1
...
for tid in ("radar-runtime:pilot-open-v6194","radar-runtime:pilot-install-v6194","radar-runtime:pilot-probe-v6194"):
    t=by.get(tid) or {}
...
probe=by.get("radar-runtime:pilot-probe-v6194") or {}
...
print("RADAR_V6194_RUNTIME_ROUTE_GUARDS=PASS")
print("RADAR_V6194_GAME_SCAN_EXECUTED=NO")
print("RADAR_V6194_COLLECTOR_MUTATION=NO")
print("RADAR_V6194_PHONE_REQUIRED_FOR_RUNTIME=NO")
PY
...
echo "RADAR_V6194_PRODUCTION_APPROVAL_STANDING=NO"
echo "RADAR_V6194_PILOT_WINDOW_CLOSED=YES"
echo "RADAR_V6194_CHACHA_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER"
echo "RADAR_V6194_CHACHA_DEPLOY=PASS"
