...
for timer in "${TIMERS[@]}"; do
  ...
  for attempt in $(seq 1 60); do
    ...
    LIST_NEXT="$(systemctl list-timers --all --no-pager --no-legend "$timer" | awk 'NF{print $1; exit}')"
    if [ -n "$NEXT" ] && [ "$NEXT" != "infinity" ] && [ "$NEXT" != "0" ]; then
      SCHEDULED=1
      break
    fi
    if [ -n "$LIST_NEXT" ] && [ "$LIST_NEXT" != "-" ]; then
      SCHEDULED=1
      break
    fi
    sleep 2
  done
  [ "$SCHEDULED" -eq 1 ] || { echo "CHACHA_DEV_V817_INSTALL=BLOCKED reason=timer_not_scheduled:$timer"; exit 3; }
  echo "CHACHA_DEV_V817_TIMER_NEXT=$timer:${NEXT:-$LIST_NEXT}"
  echo "CHACHA_DEV_V817_TIMER_LIST_NEXT=$timer:$LIST_NEXT"
  printf '%s\n' "$MONOTONIC_SET" | sed 's/^/CHACHA_DEV_V817_TIMER_MONOTONIC=/'
done
