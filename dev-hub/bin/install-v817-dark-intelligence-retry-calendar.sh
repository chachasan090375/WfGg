...
for timer in "${TIMERS[@]}"; do
  ...
  if [ "$verified" -eq 0 ]; then
    CALENDAR="$(systemctl show "$timer" -p TimersCalendar --value)"
    ...
    case "$SERVICE_STATE" in
      activating|active|deactivating)
        printf '%s' "$CALENDAR" | grep -Fq 'OnCalendar=' || { echo "CHACHA_DEV_V817_INSTALL=BLOCKED reason=calendar_not_loaded:$timer"; exit 3; }
        [ -n "$LAST_TRIGGER" ] || { echo "CHACHA_DEV_V817_INSTALL=BLOCKED reason=trigger_missing:$timer"; exit 3; }
        echo "CHACHA_DEV_V817_TIMER_TRIGGER_IN_FLIGHT=$timer:$SERVICE_STATE"
        ;;
      *)
        echo "CHACHA_DEV_V817_INSTALL=BLOCKED reason=timer_not_scheduled:$timer"
        exit 3
        ;;
    esac
  fi
