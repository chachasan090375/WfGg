...
PROJECT="wfgg-radar"
PILOT_REV="7c807206e585b3eb21321892661657398c50e947"
PILOT_BRANCH="radar-v6192-cluster-catalog-budget"
CONNECTOR_SHA="1202bc345ed8342398d255667bd783b237182776751f40e7da7ac958ee31f9da"
NATIVE_SHA="274d040f5294cb09422e5d55cc4b5335ac7739924c33dcb67b3f279645814900"
INSTALLER="radar-vps/install-v6192-pilot.sh"
PROBE="radar-vps/probe-v6192-pilot-runtime.sh"
RAW_ARCHIVE="https://codeload.github.com/chachasan090375/WfGg/tar.gz/${REV}"
WORK="$(mktemp -d /tmp/chacha-radar-v6192-deploy.XXXXXX)"
...
cleanup(){ rm -rf "$WORK"; }
die(){ echo "RADAR_V6192_CHACHA_DEPLOY=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.2 PILOT DEPLOY ==="
...
# ---------------------------------------------------------------------------
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6192-pilot.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6192-pilot.task-graph.json"
...
obj={
  ...
  "scope":{
    "pilot":"V6.19.2",
    "revision":"7c807206e585b3eb21321892661657398c50e947",
    "change":"server-cluster-catalog timeout 30s -> 75s",
    "purpose":"recover read-only recommendedSeed for targeted sequential server discovery"
  },
  "actor":"human-user",
  "source":"chat-explicit-approval",
  "statement":"J’approuve le déploiement PILOT Radar V6.19.2 avec timeout catalogue 75 s.",
...
graph={
  ...
  "generated_at":now,
  "tasks":[{
    "id":"approval:radar-v6192-pilot",
    "kind":"approval",
    "description":"Record explicit human approval for the single V6.19.2 Radar pilot window.",
...
result={
  "schema":"chacha.dev/task-result/v1",
  "project":"wfgg-radar",
  "task_id":"approval:radar-v6192-pilot",
  "status":"OK",
  "producer":"human-approval-recorder",
  "observed_at":now,
  "summary":"Human approved the single V6.19.2 Radar pilot deployment window.",
  "evidence":[{"kind":"human-approval","source":src,"digest":digest,"details":{"approval_id":"production-release","scope":"radar-v6192-pilot"}}],
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v6192-pilot-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6192-pilot-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.2 pilot approval has been consumed and is revoked after the single pilot window.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_pilot_window":True,
  "status":"REJECTED"
}
...
graph={
 ...
 "tasks":[{"id":"approval:radar-v6192-pilot-revoke","kind":"approval","description":"Revoke the consumed one-shot production approval.","owner_role":"project-owner","capabilities":[],"permission":"read","depends_on":[],"outputs":[{"type":"approval","id":"production-release"}],"verification":{"mode":"human","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True}],
 "summary":{"task_count":1,"artifact_tasks":0,"gate_tasks":0,"approval_tasks":1,"blocking_tasks":1}}
res={
 "schema":"chacha.dev/task-result/v1","project":"wfgg-radar","task_id":"approval:radar-v6192-pilot-revoke","status":"OK","producer":"human-approval-recorder","observed_at":now,
 "summary":"Consumed single-window V6.19.2 production approval revoked.","evidence":[{"kind":"human-approval","source":s,"digest":d,"details":{"approval_id":"production-release","status":"REJECTED"}}],
...
close_pilot () {
  ...
  CLOSE_ATTEMPTED=1
  local graph="$PLAN_DIR/radar-v6192-close.task-graph.json"
  python3 - "$graph" <<'PY'
import json,sys
from datetime import datetime,timezone
...
PY
  if rr="$(run_graph "$graph" RADAR_V6192_CLOSE | tail -1)"; then
    python3 - "$rr" <<'PY'
import json,sys
x=json.load(open(sys.argv[1],encoding='utf-8'))
tasks=[t for w in x.get('waves') or [] for t in w.get('tasks') or []]
ok=len(tasks)==1 and tasks[0].get('status')=='SUCCEEDED'
print('RADAR_V6192_PILOT_CLOSE='+('PASS' if ok else 'FAIL'))
raise SystemExit(0 if ok else 1)
PY
  else
    echo "RADAR_V6192_PILOT_CLOSE=FAIL"
    return 1
  fi
}
...
# ---------------------------------------------------------------------------
# 3) ChaCha DEV governed pilot: open -> install -> read-only catalog probe.
#    Close is intentionally a separate guaranteed action in the EXIT path.
# ---------------------------------------------------------------------------
PILOT_GRAPH="$PLAN_DIR/radar-v6192-pilot.task-graph.json"
...
g={
 ...
 "tasks":[
  {"id":"radar-runtime:pilot-open-v6192","kind":"runtime-deploy","description":"Open the protected Radar V6.19.2 pilot window.","owner_role":"sre-observability","capabilities":["radar-pilot-control"],"permission":"production-deploy","depends_on":[],"outputs":[{"type":"gate","id":"radar-pilot-window"}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-open"}}},
  {"id":"radar-runtime:pilot-install-v6192","kind":"runtime-deploy","description":"Install SHA-pinned V6.19.2 pilot binaries with installer rollback protection.","owner_role":"release","capabilities":["radar-pilot-control"],"permission":"production-deploy","depends_on":["radar-runtime:pilot-open-v6192"],"outputs":[{"type":"artifact","id":rev}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"pilot-install","revision":rev,"installer":installer,"expected_connector_sha256":csha,"expected_native_sha256":nsha}}},
  {"id":"radar-runtime:cluster-catalog-probe-v6192","kind":"runtime-diagnostic","description":"Read the server cluster catalog through the installed V6.19.2 pilot and surface the recommended seed.","owner_role":"sre-observability","capabilities":["radar-runtime-inspect"],"permission":"read","depends_on":["radar-runtime:pilot-install-v6192"],"outputs":[{"type":"artifact","id":"radar-cluster-catalog-probe"}],"verification":{"mode":"machine","self_certification_allowed":False,"required_evidence":["source","timestamp","digest"]},"blocking":True,"metadata":{"radar_runtime":{"action":"cluster-catalog-probe"}}}
 ],
...
PILOT_RUN="$(run_graph "$PILOT_GRAPH" RADAR_V6192_PILOT | tail -1)"
PILOT_OPENED=1
...
for tid in ("radar-runtime:pilot-open-v6192","radar-runtime:pilot-install-v6192","radar-runtime:cluster-catalog-probe-v6192"):
    t=by.get(tid) or {}
    print(tid.replace("radar-runtime:","").upper().replace("-","_")+"_STATUS="+str(t.get('status')))
probe=by.get("radar-runtime:cluster-catalog-probe-v6192") or {}
result_path=probe.get("task_result")
if not result_path:
    raise SystemExit("CATALOG_TASK_RESULT_MISSING")
r=json.load(open(result_path,encoding='utf-8'))
if r.get("status")!="OK":
    raise SystemExit("CATALOG_TASK_RESULT_NOT_OK")
ev=r.get("evidence") or []
if not ev:
    raise SystemExit("CATALOG_EVIDENCE_MISSING")
d=ev[0].get("details") or {}
print("RADAR_V6192_CATALOG_HTTP="+str(d.get("http_status")))
print("RADAR_V6192_CATALOG_ELAPSED_MS="+str(d.get("elapsed_ms")))
print("RADAR_V6192_CATALOG_VERSION="+str(d.get("catalog_version")))
print("RADAR_V6192_CATALOG_ERROR="+str(d.get("error") or "NONE"))
print("RADAR_V6192_CONFIRMED_CLUSTERS="+str(d.get("confirmed_cluster_count")))
print("RADAR_V6192_CONFIRMED_SERVERS="+str(d.get("confirmed_server_count")))
print("RADAR_V6192_FRONTIER_COUNT="+str(d.get("frontier_count")))
print("RADAR_V6192_NOISE_COUNT="+str(d.get("noise_count")))
...
if int(d.get("http_status") or 0)!=200:
    raise SystemExit("CATALOG_HTTP_NOT_200")
if not seed.get("command"):
    raise SystemExit("CATALOG_RECOMMENDED_SEED_MISSING")
print("RADAR_V6192_DISCOVERY_SEED=PASS")
PY
...
echo "RADAR_V6192_PRODUCTION_APPROVAL_STANDING=NO"
echo "RADAR_V6192_PILOT_WINDOW_CLOSED=YES"
echo "RADAR_V6192_CHACHA_PATH=PROJECT_CONTROL>SCHEDULER>RUN_CONTROLLER>RADAR_RUNTIME_ADAPTER"
echo "RADAR_V6192_CHACHA_DEPLOY=PASS"
