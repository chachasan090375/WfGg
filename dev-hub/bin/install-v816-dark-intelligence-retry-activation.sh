...
for timer in "${TIMERS[@]}"; do
  ...
  for attempt in $(seq 1 45); do
    ...
    if [ -n "$NEXT" ] && [ "$NEXT" != "infinity" ] && [ "$NEXT" != "0" ]; then break; fi
    sleep 2
  done
  [ -n "$NEXT" ] && [ "$NEXT" != "infinity" ] && [ "$NEXT" != "0" ] || { echo "CHACHA_DEV_V816_INSTALL=BLOCKED reason=timer_not_scheduled:$timer"; exit 3; }
  LIST_ROW="$(systemctl list-timers --all --no-pager --no-legend "$timer" | head -1)"
  [ -n "$LIST_ROW" ] || { echo "CHACHA_DEV_V816_INSTALL=BLOCKED reason=timer_not_listed:$timer"; exit 3; }
  echo "CHACHA_DEV_V816_TIMER_NEXT=$timer:$NEXT"
  echo "CHACHA_DEV_V816_TIMER_LISTED=$timer"
done
