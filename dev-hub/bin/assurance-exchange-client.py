...
def final_reviews(a)->int:
    _,url,key=policy_values(a.policy)
    q={"project_id":a.project_id,"revision":a.revision,"compromise_digest":a.compromise_digest}
    full=url+"/v1/final-reviews?"+urllib.parse.urlencode(q)
    try:status,x=http(signed_request("GET",full,key))
    except Exception as exc:
        print(json.dumps({"schema":"chacha.dev/assurance-exchange-client-result/v1","status":"UNAVAILABLE","reason":str(exc)[:300]}));return 30
    print(json.dumps(x,ensure_ascii=False))
...
def main()->int:
    ...
    fr=sub.add_parser("final-reviews");fr.add_argument("--project-id",required=True);fr.add_argument("--revision",required=True);fr.add_argument("--compromise-digest",required=True)
    ...
    if a.cmd=="final-reviews":return final_reviews(a)
