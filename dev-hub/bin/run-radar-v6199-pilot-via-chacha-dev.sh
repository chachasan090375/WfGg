#!/usr/bin/env bash
# V6.19.9 Autopilot Targeted History Quality guard-only pilot runner; qualification workflow must be green before execution.
# Radar runtime adapter provisioning revision: 1.5.4 (immutable version bump after digest change).
set -Eeuo pipefail

REV="${WFGG_DEV_HUB_RADAR_V6199_DEPLOY_REV:-}"
PROJECT="wfgg-radar"
...
mkdir -p "$WORK_ROOT"
WORK="$(mktemp -d "$WORK_ROOT/chacha-radar-v6199-deploy.XXXXXX")"
...
cleanup(){ rm -rf "$WORK"; }
die(){ echo "RADAR_V6199_CHACHA_DEPLOY=BLOCKED reason=$1"; exit 2; }
...
echo "=== CHACHA DEV RADAR V6.19.9 PILOT DEPLOY ==="
...
# ---------------------------------------------------------------------------
APPROVAL_EVIDENCE="$EVIDENCE_DIR/approval-radar-v6199-pilot.json"
APPROVAL_GRAPH="$PLAN_DIR/approval-radar-v6199-pilot.task-graph.json"
...
obj={
  ...
  "statement":"J’approuve le PILOT Radar V6.19.9 Autopilot Targeted History Quality révision 53ad1ca3295bca9cf45b4a06d40f4fb61a84d6de avec probe de garde sans scan de jeu.",
...
result={
  "schema":"chacha.dev/task-result/v1",
  "project":"wfgg-radar",
  "task_id":"approval:radar-v6199-pilot",
  "status":"OK",
...
revoke_approval () {
  ...
  local ev="$EVIDENCE_DIR/approval-radar-v6199-pilot-revoked.json"
  local graph="$PLAN_DIR/approval-radar-v6199-pilot-revoked.task-graph.json"
...
obj={
  ...
  "actor":"human-user",
  "source":"scope-expiry",
  "statement":"The explicit V6.19.9 Targeted History Quality pilot approval has been consumed and is revoked after the single pilot window.",
  "observed_at":datetime.now(timezone.utc).isoformat(),
  "single_pilot_window":True,
  "status":"REJECTED"
}
