...
api_post(){
  local path="$1" body="$2"
  printf '%s' "$body" | ssh "${SSH_OPTS[@]}" -T "$REMOTE" python3 - "$path" <<'PY'
import sys,urllib.request
path=sys.argv[1]
body=sys.stdin.buffer.read()
req=urllib.request.Request('http://127.0.0.1:8790'+path,data=body,method='POST',headers={'Content-Type':'application/json'})
...
api_get(){
  local path="$1"
  ssh "${SSH_OPTS[@]}" -T "$REMOTE" python3 - "$path" <<'PY' </dev/null
import sys,urllib.request
with urllib.request.urlopen('http://127.0.0.1:8790'+sys.argv[1],timeout=15) as r:
    sys.stdout.write(r.read().decode())
PY
}
...
START_JSON="$(api_post '/cycle/start' "$START_BODY")"
...
if [[ "$JOINED" == YES ]]; then
  for _ in $(seq 1 120); do
    CUR="$(api_get "/cycle/status?id=$CYCLE_ID")"
...
FIN_JSON="$(api_post '/cycle/finish' "$FIN_BODY")"
python3 - "$FIN_JSON" <<'PY'
import json,sys
